...
# ================================================================================
    ...
    def mu_max_latex_equation(params):
        ...
        """
        return (f"$\\mu_{{\\mathrm{{max}}}} = {params[0]:.3e} \\times "
    ...
    def Nmax_latex_equation(params):
        ...
        return (f"$N_{{\\mathrm{{max}}}} = {params[0]:.3e} \\times "
                f"\\frac{{L_0}}{{L_0 + {params[1]:.3e}}} \\times "
                f"\\frac{{C_0}}{{C_0 + {params[2]:.3e}}}$")
...
def print_r2_statistics_summary(df_r2, df_params_all, coeffs_mu_max, coeffs_Nmax):
    ...
    def get_params_subset_by_L0_C0(df_r2_subset, df_params_all):
        ...
        for _, r2_row in df_r2_subset.iterrows():
            ...
            matching = df_params_all[(df_params_all['L0'] == L0) & (df_params_all['C0'] == C0)]
            if len(matching) > 0:
    ...
    def compute_stats_for_subset(df_r2_subset, df_params_subset, label):
        ...
        for _, row in df_params_subset.iterrows():
            ...
            N0_values = [fit['fit']['N0'] for fit in replicate_fits
            ...
            for fit_data in replicate_fits:
                ...
                biomass_rsm = logistic_growth_rsm(time_clean, N0_mean, L0, C0,
                                                  coeffs_mu_max, coeffs_Nmax, df_params_all)
        ...
        if n_data_points > 0:
            global_r2 = calculate_r2(np.array(all_biomass_exp), np.array(all_biomass_pred))
        else:
            global_r2 = np.nan
        ...
        logger.info(f"  Conditions with R² ≥ 0.85: {n_good}/{n_conditions} ({pct_good:.1f}%)")
    ...
    calib_data, n_calib = compute_stats_for_subset(df_r2_calibration, df_params_calibration, "CALIBRATION SET")
    ...
    valid_data, n_valid = compute_stats_for_subset(df_r2_validation, df_params_validation, "VALIDATION SET (intermediate dilutions)")

    logger.info("")

    # OVERALL (calibration + validation combined)
    if n_calib > 0 and n_valid > 0 and calib_data is not None and valid_data is not None:
        # Combine all data
        ...
        if n_data_points_overall > 0:
            global_r2_overall = calculate_r2(np.array(all_exp_combined), np.array(all_pred_combined))
        else:
            global_r2_overall = np.nan

        logger.info("OVERALL (calibration + validation combined):")
        logger.info(f"  Number of conditions: {n_overall}")
        logger.info(f"  Conditions with R² ≥ 0.85: {n_good_overall}/{n_overall} ({pct_good_overall:.1f}%)")
...
def logistic_growth_rsm(t, N0, L0, C0, coeffs_mu_max, coeffs_Nmax, df_params_tlag):
    ...
    mask = (np.abs(df_params_tlag['L0'] - L0) < 1.0) & (np.abs(df_params_tlag['C0'] - C0) < 0.001)
    ...
    if len(matching_rows) > 0:
        ...
        distances = np.sqrt((df_params_tlag['L0'] - L0)**2 + (df_params_tlag['C0'] - C0)**2)
        closest_idx = distances.idxmin()
        t_lag = df_params_tlag.loc[closest_idx, 't_lag_mean']
        logger.warning(f"No exact match for L0={L0}, C0={C0}, using closest: t_lag={t_lag:.2f}")
...
def estimate_growth_parameters(time, biomass):
    ...
    if len(biomass_clean) > 5:
        ...
        for i in range(1, len(biomass_clean) - 1):
            if biomass_clean[i] > N0_guess * 1.5 and biomass_clean[i] < Nmax_guess * 0.7:
...
def apply_manual_tlag_adjustments(df_params, manual_adjustments):
    ...
    for (L0_target, C0_target), new_tlag in manual_adjustments.items():
        ...
        if len(matching_indices) > 0:
            ...
            logger.warning(f"  ⚠ Condition L0={L0_target:.1f}, C0={C0_target:.4f} not found in DataFrame!")
...
def filter_conditions(df_params, selected_conditions):
    ...
    for L0_target, C0_target in selected_conditions:
        ...
        condition_mask = (
            (np.abs(df_params['L0'] - L0_target) < tolerance_L0) &
...
def extract_parameters_with_replicates(files_dict, conv_OD_to_cell=4.77e6, selected_conditions=None):
    ...
    if selected_conditions is not None:
        ...
        logger.info(f"Filtering enabled: will process only {len(selected_set)} selected conditions")
    else:
        selected_set = None
        ...
        for exp_name, exp_data in experiments.items():
            ...
            if selected_set is not None:
                ...
                for L0_sel, C0_sel in selected_set:
                    if (np.abs(L0_absolute - L0_sel) < 1.0 and
                        np.abs(C0_factor - C0_sel) < 0.001):
                        condition_selected = True
                        break

                if not condition_selected:
                    logger.debug(f"  Skipping L0={L0_absolute:.1f}, C0={C0_factor:.3f} (not selected)")
                    continue
            ...
            for i, rep_name in enumerate(rep_names):
                ...
                if params and params['r_squared'] > 0.7:
                    ...
                    logger.warning(f"  L0={L0_absolute}, C0={C0_factor}, Rep {rep_name}: Fitting failed")
            ...
            if len(replicate_params['mu_max']) >= 2:
                ...
                logger.info(f"  ✓ L0={L0_absolute:.1f}, C0={C0_factor:.3f}: "
                ...
                logger.warning(f"  L0={L0_absolute}, C0={C0_factor}: Not enough successful fits")
...
def fit_response_surface(df, response_var_mean, degree=2):
    ...
    try:
        ...
        logger.warning(f"Fit failed for {response_var_mean}: {e}. Using initial values.")
        coeffs = np.array(p0)
        r2 = 0.0
...
def plot_3d_surface(df, response_var, output_dir, df_all_conditions=None):
    ...
    for L0_val in L0_all_unique:
        ...
        for _, row in df_L0.iterrows():
            ax1.scatter(row['L0'], row['C0'], row[response_mean],
                       c=[L0_to_color_all[L0_val]], s=50, linewidth=2, alpha=0.8)

            # Vertical error bars
            ax1.plot([row['L0'], row['L0']],
                    [row['C0'], row['C0']],
                    [row[response_mean] - row[response_std], row[response_mean] + row[response_std]],
                    color=L0_to_color_all[L0_val], linewidth=2, alpha=0.6)
    ...
    surf = ax2.plot_surface(L0_grid, C0_grid, response_grid,
    ...
    if df_all_conditions is not None:
        ...
        df_calibration = df_all_conditions[
            df_all_conditions.apply(lambda row: (row['L0'], row['C0']) in df_calib_keys, axis=1)
        ].copy()
        ...
        for idx, row in df_calibration.iterrows():
            ax2.scatter(row['L0'], row['C0'], row[response_mean],
                       color='black', s=50, marker='o', linewidth=1.5,
                       alpha=0.9,
                       label='Calibration' if idx == df_calibration.index[0] else '')
            ax2.plot([row['L0'], row['L0']],
                    [row['C0'], row['C0']],
                    [row[response_mean] - row[response_std], row[response_mean] + row[response_std]],
        ...
        for idx, row in df_extrapolation.iterrows():
            ax2.scatter(row['L0'], row['C0'], row[response_mean],
                       color='mediumslateblue', s=100, marker='o', edgecolor='white', linewidth=1.5, alpha=0.9,
                       label='Extrapolation' if idx == df_extrapolation.index[0] else '')
            ax2.plot([row['L0'], row['L0']],
                    [row['C0'], row['C0']],
                    [row[response_mean] - row[response_std], row[response_mean] + row[response_std]],
                    color='mediumslateblue', linewidth=2, alpha=0.7)
    ...
    contour = ax3.contourf(L0_grid, C0_grid, response_grid,
                          levels=15, cmap=cmap_choice, alpha=0.8)
    contour_lines = ax3.contour(L0_grid, C0_grid, response_grid,
    ...
    if df_all_conditions is not None:
        ...
        # Default behavior
        ax3.errorbar(df['L0'], df['C0'],
                    xerr=0, yerr=0,
                    fmt='o', color='black', markersize=8,
                    ecolor='gray', elinewidth=2, capsize=3, capthick=2)
    ...
    ax_eq.axis('off')
    ax_eq.text(0.5, 0.5, poly_text, fontsize=10, ha='center', va='center',
    ...
    if response_var == 'mu_max':
        ...
        gs2d = fig2d.add_gridspec(1, 2, wspace=0.25, left=0.08, right=0.98, top=0.92, bottom=0.12)
        ...
        if df_all_conditions is not None:
            ...
            for C0_val in C0_unique:
                color = colors_C0_specific.get(C0_val, 'black')  # Specific color for this C0

                # Filter data for this C0
                df_C0 = df_all_conditions[df_all_conditions['C0'] == C0_val].copy()
                
                # Separate calibration and extrapolation
                df_C0_calib = df_C0[df_C0.apply(lambda row: (row['L0'], row['C0']) in df_calib_keys, axis=1)]
                df_C0_extrap = df_C0[~df_C0.apply(lambda row: (row['L0'], row['C0']) in df_calib_keys, axis=1)]
                
                # Calibration points (CIRCLES, no edge)
                if len(df_C0_calib) > 0:
                    ax_left.errorbar(df_C0_calib['L0'], df_C0_calib[response_mean],
                ...
                if len(df_C0_extrap) > 0:
                    ax_left.errorbar(df_C0_extrap['L0'], df_C0_extrap[response_mean],
                ...
                ax_left.plot(L0_curve, response_curve, '-', color=color, linewidth=2,
                            label=f'$C_0$ = {C0_val:.4f}', alpha=0.9)
        ...
        calib_marker = Line2D([0], [0], marker='o', color='w',
                             markerfacecolor='grey', markersize=8,
                             linestyle='none', label='Calibration')
        extrap_marker = Line2D([0], [0], marker='^', color='w',
                              markerfacecolor='grey', markersize=10,
                              linestyle='none', label='Extrapolation')
        ...
        predict_func_other, coeffs_other, r2_other = fit_response_surface(df, other_mean)
        ...
        if df_all_conditions is not None:
            for C0_val in C0_unique:
                ...
                # Calibration points (CIRCLES, no edge)
                if len(df_C0_calib) > 0:
                    ax_right.errorbar(df_C0_calib['L0'], df_C0_calib[other_mean],
                ...
                # Extrapolation points (TRIANGLES, no edge)
                if len(df_C0_extrap) > 0:
                    ax_right.errorbar(df_C0_extrap['L0'], df_C0_extrap[other_mean],
                ...
                L0_curve = np.linspace(L0_min, L0_max, 100)
                response_curve_other = np.array([predict_func_other(L0, C0_val) for L0 in L0_curve])
                
                ax_right.plot(L0_curve, response_curve_other, '-', color=color, linewidth=2, alpha=0.9)
...
def plot_fits_overview(df, output_dir):
    ...
    for idx, row in df.iterrows():
        ...
        for fit_data in row['replicate_fits']:
            ...
            # Experimental data
            ax.plot(time, biomass, 'o',
                   color=colors_rep[rep_name], markersize=5, alpha=0.6)

            # Fit
            ax.plot(fit['time_fit'], fit['biomass_fit'], '-', color=colors_rep[rep_name], linewidth=1.5, label=f'{rep_name}: R²={fit["r_squared"]:.2f}')

        # Title with ALL parameters and their standard deviations
        title_text = (f"L0={L0:.0f} µmol/m²/s, C0={C0:.3f}\n"
...
def plot_fits_overview_rsm(df, coeffs_mu_max, coeffs_Nmax, df_params_tlag, output_dir):
    ...
    for idx, row in df.iterrows():
        ...
        for fit_data in row['replicate_fits']:
            ...
            # Individual fit
            ax.plot(fit['time_fit'], fit['biomass_fit'], '-',
                   color=colors_rep[rep_name], linewidth=1.5, alpha=0.7,
        ...
        if N0_estimate is not None and t_max > 0:
            ...
            if mu_max_rsm > 0 and Nmax_rsm > 0:
                ...
                try:
                    ...
                    ax.plot(time_smooth, biomass_rsm, '-',
                           color='black', linewidth=2.5, alpha=0.9,
                           label=f'RSM-Logistic: R²={r2_rsm:.2f}', zorder=10)
                    
                    logger.info(f"RSM curve plotted for L0={L0:.1f}, C0={C0:.3f}: "
                               f"mu_max={mu_max_rsm:.3f}, Nmax={Nmax_rsm:.2e}, t_lag={t_lag_rsm:.2f}, R²={r2_rsm:.3f}")
                except Exception as e:
                    logger.warning(f"Failed to plot RSM curve for L0={L0:.1f}, C0={C0:.3f}: {e}")
            else:
                logger.warning(f"Invalid RSM parameters for L0={L0:.1f}, C0={C0:.3f}: "
                              f"mu_max={mu_max_rsm:.3f}, Nmax={Nmax_rsm:.2e}")
        else:
            logger.warning(f"No valid data for RSM plot: L0={L0:.1f}, C0={C0:.3f}, "
...
def evaluate_rsm_extrapolation(df_params_selected, coeffs_mu_max, coeffs_Nmax, df_params_tlag,
                                files_dict, conv_OD_to_cell, output_dir):
    """
    ...
    logger.info(f"Using {len(df_params_all)} conditions (with manual t_lag adjustments applied)")
    ...
    for idx, row in df_params_all.iterrows():
        ...
        N0_values = [fit['fit']['N0'] for fit in replicate_fits if 'fit' in fit and fit['fit'] is not None]
        if not N0_values:
            continue
        N0_mean = np.mean(N0_values)
    ...
    if len(valid_r2) > 0:
        ...
        if len(all_biomass_exp) > 0:
            r2_global = calculate_r2(np.array(all_biomass_exp), np.array(all_biomass_pred))
        ...
        logger.info(f"    L0 = {condition_min['L0']:.1f} µmol/m²/s, C0 = {condition_min['C0']:.4f}")
        logger.info(f"    R² = {condition_min['R2_rsm_mean']:.6f} ± {condition_min['R2_rsm_std']:.6f}")
        logger.info(f"    {'[USED FOR FIT]' if condition_min['used_for_fit'] else '[EXTRAPOLATION]'}")

        logger.info(f"\n  Condition with maximum R²:")
        logger.info(f"    L0 = {condition_max['L0']:.1f} µmol/m²/s, C0 = {condition_max['C0']:.4f}")
        logger.info(f"    R² = {condition_max['R2_rsm_mean']:.6f} ± {condition_max['R2_rsm_std']:.6f}")
        logger.info(f"    {'[USED FOR FIT]' if condition_max['used_for_fit'] else '[EXTRAPOLATION]'}")
    else:
    ...
    for idx, row in df_params_all.iterrows():
        ...
        manually_adjusted = (L0, C0) in manual_tlag_adjustments if 'manual_tlag_adjustments' in locals() else False
        
        logger.info(f"{L0:<18.1f} {C0:<10.4f} {t_lag_mean:<12.2f} {status:<15}{'[MANUAL]' if manually_adjusted else ''}")
    ...
    df_params_all = df_params_all.sort_values(['L0', 'C0'], ascending=[False, True]).reset_index(drop=True)
    ...
    for idx, row in df_params_all.iterrows():
        ...
        L0, C0, replicate_fits, used_for_fit = row['L0'], row['C0'], row['replicate_fits'], row['used_for_fit']
        ...
        t_max = 0
        for fit_data in replicate_fits:
            rep_name, time, biomass = fit_data['replicate'], fit_data['time'], fit_data['biomass']
            t_max = max(t_max, np.nanmax(time))
            ax.plot(time, biomass, 'o', color=colors_rep[rep_name], alpha=0.7, markersize=5)
            fit = fit_data['fit']
            ax.plot(fit['time_fit'], fit['biomass_fit'], '-', color=colors_rep[rep_name], alpha=0.7)
        
        # RSM curve
        if t_max > 0:
            time_rsm = np.linspace(0, t_max, 500)
            biomass_rsm = logistic_growth_rsm(time_rsm, N0_mean, L0, C0,
                                              coeffs_mu_max, coeffs_Nmax, df_params_all)
            color = 'black' if used_for_fit else 'mediumslateblue'
            ax.plot(time_rsm, biomass_rsm, color=color, linewidth=2.5,
                    label='RSM (fit)' if used_for_fit else 'RSM (extrap)', zorder=10)
            
            # Log the t_lag value used
            if idx == 0 or idx % 5 == 0:  # Display every 5 to avoid overloading
                logger.debug(f"  Condition L0={L0:.1f}, C0={C0:.4f}: using t_lag={t_lag_mean:.2f} h")
        
        # Title
        fit_status = "fit" if used_for_fit else "extrapolation"
        ax.set_title(f"L0={L0:.1f}, C0={C0:.3f} [{fit_status}]\nR²(RSM)={r2_rsm:.3f}±{r2_rsm_std:.3f}", fontsize=14, fontweight='bold' if not used_for_fit else 'normal', color='mediumslateblue' if not used_for_fit else 'black')
        #ax.set_ylim(0, 6e7)
...
def evaluate_rsm_extrapolation_by_L0(df_params_selected, coeffs_mu_max, coeffs_Nmax, df_params_tlag,
                                      files_dict, conv_OD_to_cell, output_dir):
    """
    ...
    # L0 values
    L0_values = sorted(df_params_all['L0'].unique(), reverse=True)  # [170, 102, 51, 25.5, 11.9]
    ...
    logger.info(f"Scaled Y-axis range: {y_min_plot_scaled:.2f} to {y_max_plot_scaled:.2f} × 10^7 cells/mL")
    
    # =========================================================================
    # CREATE THE FIGURE
    # =========================================================================
    fig, axes = plt.subplots(1, 5, figsize=(36, 7))  # Increased from 32 to 36 for more space
    ...
    for idx_L0, L0 in enumerate(L0_values):
        ...
        for idx_row, row in df_L0.iterrows():
            ...
            for fit_data in replicate_fits:
                ...
                ax.plot(time, biomass / 1e7, marker=marker, color=color, alpha=0.6,
            ...
            if t_max > 0:
                ...
                ax.plot(time_rsm, biomass_rsm / 1e7, color=color, linewidth=2.5,
                       linestyle=linestyle, label=f'C0={C0:.4f}', alpha=0.9)
        
        # Plot configuration
        # Title with LaTeX notation and BOLD
        ax.set_title(rf'$L_{{0}}$ = {L0:.1f} µmol$_{{h\nu}}$ m$^{{-2}}$ s$^{{-1}}$',
...
def plot_r2_heatmap_grid(df_params_all, coeffs_mu_max, coeffs_Nmax, df_params_tlag, output_dir):
    """
    ...
    df_sorted = df_params_all.sort_values(['L0', 'C0'], ascending=[False, False]).reset_index(drop=True)
    
    # Create unique L0 and C0 values for the axes
    L0_values = sorted(df_sorted['L0'].unique(), reverse=True)  # [170, 102, 51, 25.5, 11.9]
    C0_values = sorted(df_sorted['C0'].unique(), reverse=False)  # [1.0, 0.5, 0.25, 0.125, 0.0625]
    ...
    for i, L0 in enumerate(L0_values):
        for j, C0 in enumerate(C0_values):
            ...
            x_base = j * cell_size
            y_base = (len(L0_values) - 1 - i) * cell_size  # Invert Y to have L0 max at the top
            
            # The 4 sub-squares: [A (TL), B (TR), C (BL), RSM (BR)]
            # TL = Top Left, TR = Top Right, BL = Bottom Left, BR = Bottom Right
            sub_positions = [
                (x_base, y_base + cell_size/2, cell_size/2, cell_size/2),  # A: top-left
                (x_base + cell_size/2, y_base + cell_size/2, cell_size/2, cell_size/2),  # B: top-right
                (x_base, y_base, cell_size/2, cell_size/2),  # C: bottom-left
                (x_base + cell_size/2, y_base, cell_size/2, cell_size/2)   # RSM: bottom-right
            ]
            ...
            for k, (x, y, w, h) in enumerate(sub_positions):
                ...
                if not np.isnan(r2_val):
                    ...
                    # Label + valeur
                    ax.text(text_x, text_y + 0.08, sub_labels[k],
                           ha='center', va='center', fontsize=7,
                           fontweight='bold', color=text_color)
                    ax.text(text_x, text_y - 0.08, f'{r2_val:.2f}',
                    ...
                    rect = plt.Rectangle((x, y), w, h,
                                        ...
                                        linewidth=1.5)
                    ax.add_patch(rect)
                    ax.text(x + w/2, y + h/2, 'N/A',
                           ha='center', va='center', fontsize=8,
                           color='darkgray')
            
            # Add separation lines between main cells
            rect_main = plt.Rectangle((x_base, y_base), cell_size, cell_size,
                                     fill=False, edgecolor='white', linewidth=4)
    ...
    ax.set_yticklabels([f'{L:.1f}' for L in reversed(L0_values)], fontsize=11, fontweight='bold')
    ax.set_ylabel('L₀ (µmol/m²/s)', fontsize=14, fontweight='bold')
    
    # Title
    ax.set_title('R² Heatmap Grid: Individual Replicates (A, B, C) and RSM-ODE Hybrid Model\n' +
    ...
    for k, label in enumerate(['Replicate A', 'Replicate B', 'Replicate C', 'RSM-ODE Model']):
...
def create_full_rsm_panel(df, output_dir, df_all_conditions=None):
    ...
    if df_all_conditions is not None:
        # mu_max global R²
        mu_pred_all = np.array([predict_func_mu(row['L0'], row['C0']) for _, row in df_all_conditions.iterrows()])
        ...
        # Nmax global R²
        N_pred_all = np.array([predict_func_N(row['L0'], row['C0']) for _, row in df_all_conditions.iterrows()])
    ...
    C0_unique = sorted(df_all_conditions['C0'].unique()) if df_all_conditions is not None else sorted(df['C0'].unique())
    ...
    gs = gridspec.GridSpec(3, 4,
                          ...
                          left=0.06, right=0.94, top=0.97, bottom=0.08)  # More space at bottom for colorbars
    ...
    surf_mu = ax_3d_mu.plot_surface(L0_grid, C0_grid, mu_grid,
    ...
    if df_all_conditions is not None:
        ...
        for idx, row in df_calibration.iterrows():
            ax_3d_mu.scatter(row['L0'], row['C0'], row['mu_max_mean'],
                           color='black', s=50, marker='o', alpha=0.9)
            # Vertical error bars
            ax_3d_mu.plot([row['L0'], row['L0']],
                         [row['C0'], row['C0']],
        ...
        for idx, row in df_extrapolation.iterrows():
            ...
            ax_3d_mu.plot([row['L0'], row['L0']],
                         [row['C0'], row['C0']],
                         [row['mu_max_mean'] - row['mu_max_std'], row['mu_max_mean'] + row['mu_max_std']],
                         color='mediumslateblue', linewidth=1.5, alpha=0.6)
    
    ax_3d_mu.set_xlabel(r'$L_0$ (µmol$_{h\nu}$ m$^{-2}$ s$^{-1}$)',
                        fontsize=FONT_LABEL_3D, labelpad=10)
    ...
    ax_3d_mu.set_title(f'R² = {r2_mu:.3f} (calibration), R² = {r2_mu_global:.3f} (global)', fontsize=FONT_TITLE_3D, pad=10)
    ax_3d_mu.tick_params(labelsize=FONT_TICK_3D)
    
    # Colorbar for mu_max
    cbar_mu = plt.colorbar(surf_mu, ax=ax_3d_mu, shrink=0.5, aspect=10, pad=0.1)
    cbar_mu.set_label(r'$\mu_{\mathrm{max}}$ (h$^{-1}$)',
                      fontsize=FONT_COLORBAR, rotation=270, labelpad=20)
    cbar_mu.ax.tick_params(labelsize=FONT_TICK_3D)
    
    # --- 3D Surface Nmax ---
    ax_3d_N = fig.add_subplot(gs[0, 2:4], projection='3d')
    
    surf_N = ax_3d_N.plot_surface(L0_grid, C0_grid, N_grid,
    ...
    if df_all_conditions is not None:
        for idx, row in df_calibration.iterrows():
            ax_3d_N.scatter(row['L0'], row['C0'], row['Nmax_mean'],
                          color='black', s=50, marker='o', alpha=0.9)
            # Vertical error bars
            ax_3d_N.plot([row['L0'], row['L0']],
                        [row['C0'], row['C0']],
    ...
    ax_3d_N.set_zlabel(r'$N_{\mathrm{max}}$ (cells mL$^{-1}$ × $10^7$)', fontsize=FONT_LABEL_3D, labelpad=10)
    ax_3d_N.set_title(f'R² = {r2_N:.3f} (calibration), R² = {r2_N_global:.3f} (global)', fontsize=FONT_TITLE_3D, pad=10)
    ...
    cbar_N.set_label(r'$N_{\mathrm{max}}$ (cells mL$^{-1}$ × $10^7$)',
                     fontsize=FONT_COLORBAR, rotation=270, labelpad=20)
...
# =========================================================================
    ...
    if df_all_conditions is not None:
        for C0_val in C0_unique:
            ...
            # Calibration points
            if len(df_C0_calib) > 0:
                ax_2d_mu.errorbar(df_C0_calib['L0'], df_C0_calib['mu_max_mean'],
            ...
            if len(df_C0_extrap) > 0:
                ax_2d_mu.errorbar(df_C0_extrap['L0'], df_C0_extrap['mu_max_mean'],
            ...
            ax_2d_mu.plot(L0_curve, mu_curve, '-', color=color, linewidth=2,
                        label=f'$C_0$ = {C0_val:.4f}', alpha=0.9)
    ...
    legend1 = ax_2d_mu.legend(handles_C0, labels_C0,
                              fontsize=FONT_LEGEND,
                              framealpha=0.95,
                              loc='upper left',
                              title='$C_0$ values',
                              title_fontsize=FONT_LEGEND, frameon = False)
    ...
    legend2 = ax_2d_mu.legend(handles=[calib_marker, extrap_marker],
    ...
    if df_all_conditions is not None:
        for C0_val in C0_unique:
            ...
            # Calibration points
            if len(df_C0_calib) > 0:
                ax_2d_N.errorbar(df_C0_calib['L0'], df_C0_calib['Nmax_mean'],
            ...
            # Extrapolation points
            if len(df_C0_extrap) > 0:
                ax_2d_N.errorbar(df_C0_extrap['L0'], df_C0_extrap['Nmax_mean'],
    ...
    ax_2d_N.set_ylabel(r'$N_{\mathrm{max}}$ (cells mL$^{-1}$ × $10^7$)', fontsize=FONT_LABEL_2D)
    ...
    dmu_dL0 = beta0_mu * (KL_mu / L0_max_norm / (L0_mesh_norm + KL_mu / L0_max_norm)**2) * (C0_mesh / (C0_mesh + KC_mu))
    dmu_dC0 = beta0_mu * (L0_mesh_norm / (L0_mesh_norm + KL_mu / L0_max_norm)) * (KC_mu / (C0_mesh + KC_mu)**2)
    dN_dL0 = beta0_N * (KL_N / L0_max_norm / (L0_mesh_norm + KL_N / L0_max_norm)**2) * (C0_mesh / (C0_mesh + KC_N))
    dN_dC0 = beta0_N * (L0_mesh_norm / (L0_mesh_norm + KL_N / L0_max_norm)) * (KC_N / (C0_mesh + KC_N)**2)
    ...
    sensitivity_data = [
        (dmu_dL0, r"$\partial\mu_{\mathrm{max}}/\partial L_0$", vmin_mu, vmax_mu, 'YlOrBr', 'black'),
        (dmu_dC0, r"$\partial\mu_{\mathrm{max}}/\partial C_0$", vmin_mu, vmax_mu, 'YlOrBr', 'black'),
        (dN_dL0, r"$\partial N_{\mathrm{max}}/\partial L_0$", vmin_N, vmax_N, 'YlGnBu', 'black'),
        (dN_dC0, r"$\partial N_{\mathrm{max}}/\partial C_0$", vmin_N, vmax_N, 'YlGnBu', 'white'),
    ]
    ...
    for idx, (field, title, vmin, vmax, cmap, contour_color) in enumerate(sensitivity_data):
        ...
        cont = ax.contour(L0_sens, C0_sens, field, levels=cont_levels,
                         colors=contour_color, linewidths=1.5, alpha=0.7)
        
        labels = ax.clabel(cont, inline=True, fontsize=FONT_CONTOUR, fmt='%.1e',
                          inline_spacing=15, colors=contour_color)
...
def main():
    ...
    if selected_conditions is not None:
        logger.info(f"MODE: FILTERED ANALYSIS ({len(selected_conditions)} conditions selected)")
    else:
    ...
    logger.info(f"   Extracted {len(df_params_all_25)} conditions total (all t_lag values available)")
    ...
    if selected_conditions is not None:
        ...
        logger.info(f"   Using {len(df_params_for_rsm)} conditions for RSM surface fitting")
    else:
    ...
    if manual_tlag_adjustments:
        df_params_all_25 = apply_manual_tlag_adjustments(df_params_all_25, manual_tlag_adjustments)
    ...
    df_params.drop(columns=['replicate_fits']).to_csv(csv_path, index=False, float_format='%.6f', sep=';')
    ...
    df_params_all_25_export['type'] = df_params_all_25_export.apply(
        lambda row: 'calibration' if (row['C0'], row['L0']) in calib_keys else 'extrapolation', axis=1)
    csv_path_all = output_dir / "growth_parameters_all_25_conditions.csv"
    df_params_all_25_export.to_csv(csv_path_all, index=False, float_format='%.6f', sep=';')
    ...
    for param in ['mu_max', 'Nmax', 't_lag']:
        logger.info(f"\n{param}_mean:")
        logger.info(f"  Overall Mean ± Std: {df_params[f'{param}_mean'].mean():.4e} ± {df_params[f'{param}_mean'].std():.4e}")
        logger.info(f"  Range: [{df_params[f'{param}_mean'].min():.4e}, {df_params[f'{param}_mean'].max():.4e}]")
        logger.info(f"  Avg StdDev across replicates: {df_params[f'{param}_std'].mean():.4e}")
    ...
    for response_var in ['mu_max', 'Nmax']:  # We remove 't_lag' from this loop
        ...
        coeffs = plot_3d_surface(df_params, response_var, output_dir, df_all_conditions=df_params_all_25)
    ...
    fig = create_full_rsm_panel(df_params, output_dir, df_all_conditions=df_params_all_25)
    ...
    plot_fits_overview_rsm(df_params,
                          coeffs_dict['mu_max'],
                          coeffs_dict['Nmax'],
    ...
    logger.info("\nGenerated files:")
    logger.info("  - growth_parameters_with_replicates.csv (includes individual A, B, C values)")
    logger.info("  - RSM_3D_mu_max.png, RSM_3D_Nmax.png (3D surface plots with error bars)")
    ...
    logger.info("  - rsm_extrapolation_all_conditions.png (RSM fits on all 25 conditions)")
    logger.info("  - rsm_extrapolation_r2.csv (R² values for fit and extrapolation conditions)")
    logger.info("  - r2_heatmap_grid_5x5.png (R² heatmap grid: replicates A/B/C and RSM-ODE)")
