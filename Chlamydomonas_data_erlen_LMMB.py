    ...
    for L0_factor, filepath in files_dict.items():
        ...
        for cond_idx in range(5):
            ...
            all_data[(Ein_val, C0_val)] = {
                ...
                "mean": np.mean(
                    [data[cond_idx]["A"], data[cond_idx]["B"], data[cond_idx]["C"]],
                    axis=0,
                ),
                "std": np.std(
                    [data[cond_idx]["A"], data[cond_idx]["B"], data[cond_idx]["C"]],
...
def estimate_growth_parameters(time, biomass):
    ...
    try:
        popt, _ = curve_fit(
            ...
            bounds=(
                [N0_guess * 0.1, 0.001, Nmax_guess * 0.5],
...
def plot_MR_model_results(all_data, mu, lambda_L, Gamma, output_dir="results_erlen"):
    ...
    for idx, (L0_val, C0_val) in enumerate(sorted_keys):
        ...
        N0_cond = N_plot[0]
        L0_idx = np.argmin(
            np.abs(Ein_values - L0_val)
        )  # Ein_values can remain for lookup
...
def plot_Full_model_results(
    ...
    for idx_L0, L0_val in enumerate(L0_unique):
        ...
        for _, C0_val in conditions:
            ...
            for rep_idx, (rep_name, biomass_rep) in enumerate(
                zip(replicate_names, replicates)
            ):
                # Remove NaN
    ...
    for C0, color in sorted(colors_C0.items(), reverse=True):
        legend_elements.append(
            Line2D([0], [0], color=color, linewidth=2.5, label=f"C₀={C0:.4f}")
        )
    ...
    fig.suptitle(
        rf"LMMB model fit  |  Global $R^{{2}}$ = {R2_global:.3f}",
        fontsize=18,
        fontweight="bold",
        y=1.02,
    )
...
def main():
    ...
    alpha, xi, R2_global = estimate_Full_parameters(
        all_data, mu, lambda_L, Gamma, verbose=True
    )
    ...
    plot_Full_model_results(
        all_data, mu, lambda_L, Gamma, alpha, xi, R2_global=R2_global
    )
    ...
    logger.info(
        "  2. Full_model_fit_all_conditions.png - Full model fit (all conditions)"
    )
