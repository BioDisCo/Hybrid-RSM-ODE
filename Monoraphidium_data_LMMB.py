...
def read_Kambe_data(data_folder="data_Kambe"):
    ...
    for fname in filenames:
        ...
        sorted_exps = sorted(
            experiments.values(), key=lambda e: e["C0_factor"], reverse=True
        )
...
def estimate_MR_parameters(time_data, od_data, verbose=True):
    ...
    light_indices = [
        i // 4 for i in indices_C0_1
    ]  # correctly map to light: [0, 1, 2, 3]
    ...
    result = differential_evolution(
        objective, bounds, maxiter=200, popsize=15, seed=42, atol=1e-6, tol=1e-6
    )
    ...
    if verbose:
        ...
        print(
            f"  λ_L = {lambda_L_opt:.4e} μE s⁻¹ cell⁻¹ (ref: {PARAMS_REF['lambda_L']:.2e})"
        )
...
def estimate_Full_parameters(time_data, od_data, mu, lambda_L, Gamma, verbose=True):
    ...
    result = differential_evolution(
        objective, bounds, maxiter=150, popsize=15, seed=42, atol=1e-6, tol=1e-6
    )
...
def plot_results(time_data, od_data, mu, lambda_L, Gamma, alpha, xi):
    ...
    for C0_val in sorted(colors_C0.keys(), reverse=True):
        ...
        legend_elements.append(
            Line2D([0], [0], color=color, linewidth=2.5, label=f"$C_0={C0_val:.4f}$")
        )
    ...
    for idx, light_idx in enumerate(light_indices):
        ...
        od_sim = sol.y[0, :] / (ODN * vol)
        ax_mr.plot(
            sol.t, od_sim, color=colors_mr[idx], linestyle="-", linewidth=2.5, alpha=0.8
        )
    ...
    plt.savefig(
        "results_Monoraphidium_data/fitting_LMMB.png", dpi=300, bbox_inches="tight"
    )
...
def create_results_table(mu, lambda_L, Gamma, alpha, xi):
    ...
    df.to_csv(
        "results_Monoraphidium_data/parameters_comparison_paper_vs_reproduction.csv",
        index=False,
        sep=";",
        encoding="utf-8-sig",
    )
