...
def estimate_growth_parameters(time, od):
    ...
    try:
        popt, _ = curve_fit(logistic_growth, t, y, p0=p0, bounds=(lb, ub), maxfev=5000)
        N0_fit, Nmax_fit, mu_max_fit, t_lag_fit = popt
        r2 = calculate_r2(y, logistic_growth(t, *popt))
        return dict(
            N0=N0_fit, Nmax=Nmax_fit, mu_max=mu_max_fit, t_lag=t_lag_fit, r_squared=r2
        )
...
def fit_all_conditions(records):
    ...
    for rec in records:
        ...
        if fit is not None:
            ...
            logger.warning(
                f"  L0={rec['L0']:.1f}  C0={rec['C0']:.4f}: individual fit FAILED"
            )
...
def compute_rsm_predictions(records, coeffs_mu_max, coeffs_Nmax):
    ...
    for rec in records:
        ...
        od_rsm = logistic_growth_rsm(
            t_clean, N0, L0, C0, coeffs_mu_max, coeffs_Nmax, t_lag
        )
    ...
    for _, row in passing.iterrows():
        logger.info(
            f"  PASS  L0={row['L0']:7.1f}  C0={row['C0']:.4f}  R^2={row['R2_rsm']:.4f}"
        )
...
def plot_growth_curves(
    records, coeffs_mu_max, coeffs_Nmax, df_r2, global_r2, output_dir
):
    """
        ...
        for C0 in C0_values:
            ...
            rec = next(
                (
                    r
                    for r in records
                    if abs(r["L0"] - L0) < 5 and abs(r["C0"] - C0) < 0.01
                ),
                None,
            )
            if rec is None:
            ...
            if rec["fit"] is not None:
                ...
                r2_row = df_r2[
                    (abs(df_r2["L0"] - L0) < 5) & (abs(df_r2["C0"] - C0) < 0.01)
                ]
...
def plot_individual_fits(records, output_dir):
    ...
    for ax, L0 in zip(axes, L0_values):
        for C0 in C0_values:
            ...
            if rec["fit"] is not None:
                f = rec["fit"]
                od_local = logistic_growth(
                    t_sim, f["N0"], f["Nmax"], f["mu_max"], f["t_lag"]
                )
                ax.plot(
                    t_sim,
                    od_local,
...
def plot_rsm_surfaces(df_params, coeffs_mu_max, coeffs_Nmax, output_dir):
    ...
    gs = GridSpec(2, 2, figure=fig, height_ratios=[5, 1], hspace=0.35, wspace=0.3)
    ...
    ax1.scatter(
        df_params["L0"], df_params["C0"], df_params["mu_max"], c="black", s=40, zorder=6
    )
    ...
    eq_mu = (
        r"$\mu_{\mathrm{max}}$ "
        r"$= "
        + f"{coeffs_mu_max[0]:.3e}"
        + r" \cdot \frac{L_0}{L_0 + "
        + f"{K_L_mu_real:.3e}"
        + r"} \cdot \frac{C_0}{C_0 + "
        + f"{coeffs_mu_max[2]:.3e}"
        + r"}$"
    )
    ax_eq1.axis("off")
    ax_eq1.text(
    ...
    ax2.scatter(
        df_params["L0"], df_params["C0"], df_params["Nmax"], c="black", s=40, zorder=6
    )
...
def main():
    ...
    df_params.to_csv(
        OUTPUT_DIR / "individual_fits_Monoraphidium_data.csv", index=False, sep=";"
    )
    logger.info(
        f"  -> {len(df_params)} successful fits  "
    ...
    logger.info(
        f"  RSM parameters saved to {OUTPUT_DIR}/rsm_surface_params_Monoraphidium_data.csv"
    )
