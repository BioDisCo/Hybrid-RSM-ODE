...
def calculate_specific_growth_rate(time: np.ndarray, concentration: np.ndarray, 
                                 time_window: Tuple[float, float] = None) -> float:
    """
    ...
    try:
        slope, intercept, r_value, p_value, std_err = stats.linregress(time_valid, ln_concentration)
...
def find_exponential_phase(time: np.ndarray, concentration: np.ndarray) -> Tuple[float, float]:
    """
    ...
    window_size = max(5, len(time_valid) // 6)  # Smaller windows for more precise detection
    best_r2 = 0
    best_start_idx = 0
    best_end_idx = len(time_valid) - 1
    
    # Prioritize earlier start times by adding a bias
    for start_idx in range(min(len(time_valid) // 3, len(time_valid) - window_size)):  # Start in first third
        for window_len in range(window_size, min(max_end_idx - start_idx + 1, len(time_valid) - start_idx + 1)):
            ...
            try:
                ...
                if slope > 0:  # Only positive growth rates
                    ...
                    early_bias = 1.2 - (start_idx / len(time_valid))  # Earlier = higher bias
...
def calculate_theoretical_growth_rates(c0_factors: list, base_c0: float, mu: float, gamma: float) -> list:
    """
...
def calculate_logistic_growth_rates(c0_factors: list, base_c0: float, mu: float) -> list:
    """
...
def fit_monod_parameters(c0_concentrations: list, experimental_mu: list) -> tuple:
    ...
    try:
        ...
        gamma_guess = np.median(c0_concentrations)  # Median concentration as initial guess
        ...
        # Calculate R²
        predicted_mu = monod_model(np.array(c0_concentrations), fitted_mu_max, fitted_gamma)
...
def main():
    ...
    conv_OD_to_cell = 4.46e6  # OD to cells/mL conversion factor
    ...
    if len(growth_rates) > 1:
        ...
        if fitted_mu_max is not None:
            ...
            fitted_theoretical_mu = calculate_theoretical_growth_rates(c0_values, base_C0, mu_max, Gamma)
        ...
        logistic_theoretical_mu = calculate_logistic_growth_rates(c0_values, base_C0, logistic_mu_param)
        ...
        C_smooth = np.linspace(C_min, C_max, 100)  # Linear spacing within experimental range
        ...
        mu_smooth_logistic = logistic_mu_param * C_smooth / base_C0  # Linear relationship
        ...
        ax2.set_title('Growth Rate vs Nutrient Concentration (Cell Counts)', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, 'growth_rate_vs_nutrient_concentration_with_theory.png'), dpi=300, bbox_inches='tight')
        plt.show()
        ...
        # Print comparison
        print("\n" + "="*130)
        print("COMPARISON: EXPERIMENTAL vs ORIGINAL vs FITTED vs LOGISTIC THEORETICAL GROWTH RATES")
        print("="*130)
        print(f"{'Condition':<12} {'C₀ (cells/mL)':<15} {'Experimental μ':<13} {'Original μ':<13} {'Fitted μ':<13} {'Logistic μ':<13} {'Fitted Diff':<12}")
        print("-"*130)
        for i, condition in enumerate(sorted_conditions):
            ...
            print(f"C0*{c0_factor:<7} {c0_cells:<15.2e} {exp_mu:<13.4f} {orig_mu:<13.4f} {fitted_mu:<13.4f} {logistic_mu:<13.4f} {fitted_diff:<12.4f}")
        ...
        if fitted_mu_max is not None:
            ...
            print(f"  μ_max = {fitted_mu_max:.4f} ± {param_errors[0]:.4f} h⁻¹")
            print(f"  Γ = {fitted_gamma:.2e} ± {param_errors[1]:.2e} cells/mL")
            print(f"  Base C₀ = {base_C0:.0e} cells/mL (reference nutrient concentration)")
            
            # Physical interpretation
            if fitted_gamma > base_C0:
                print(f"\nPhysical Interpretation:")
                print(f"  Γ/C₀_max = {fitted_gamma/base_C0:.2f} - Nutrient limitation is significant")
                print(f"  Growth becomes nutrient-limited when C < {fitted_gamma:.1e} cells/mL")
            else:
                print(f"\nPhysical Interpretation:")
                print(f"  Γ/C₀_max = {fitted_gamma/base_C0:.2f} - Nutrient limitation is moderate")
                print(f"  Growth becomes nutrient-limited when C < {fitted_gamma:.1e} cells/mL")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, 'individual_growth_curves_cells.png'), dpi=300, bbox_inches='tight')
        plt.show()
        
        print(f"Individual growth curves (in cells/mL) saved as '{os.path.join(results_dir, 'individual_growth_curves_cells.png')}'")
        ...
        plt.title('Steady-State Cell Concentrations vs Dilution Factor\n(Chlamydomonas reinhardtii)', fontsize=14)
        plt.grid(True, alpha=0.3)
        plt.legend()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, 'steady_states_vs_dilution_factor.png'), dpi=300, bbox_inches='tight')
        plt.show()
        ...
        print("="*90)
        print(f"{'Condition':<12} {'Dilution Factor':<16} {'Steady State (cells/mL)':<20} {'SS Ratio':<10} {'Expected':<10}")
        print("-"*90)
        
        for i, (condition, df, ss) in enumerate(zip(condition_labels, dilution_factors, steady_state_concentrations)):
            if i == 0:
                # First condition (highest concentration) - no ratio to calculate
                print(f"{condition:<12} {df:<16.3f} {ss:<20.2e} {'---':<10} {'---':<10}")
            else:
                ...
                print(f"{condition:<12} {df:<16.3f} {ss:<20.2e} {ss_ratio:<10.3f} {expected_ratio:<10.3f}")
