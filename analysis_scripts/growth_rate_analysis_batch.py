...
from growth_rate_analysis import (
    ...
    fit_monod_parameters,
    monod_model,
    calculate_theoretical_growth_rates,
    calculate_logistic_growth_rates,
)
...
def analyze_single_dataset(filepath: str, light_intensity: int, date: str) -> Dict:
    ...
    try:
        ...
        logistic_theoretical_mu = calculate_logistic_growth_rates(
            c0_values, base_C0, fitted_mu_max if fitted_mu_max else mu_max
        )
...
def generate_dataset_plots(
    ...
    plt.tight_layout()
    growth_rate_plot_path = os.path.join(
        results_dir, f"growth_rate_vs_concentration_{date}.png"
    )
    ...
    plt.tight_layout()
    individual_curves_path = os.path.join(
        results_dir, f"individual_growth_curves_{date}.png"
    )
    ...
    plt.tight_layout()
    steady_states_path = os.path.join(
        results_dir, f"steady_states_vs_dilution_{date}.png"
    )
...
def main():
    ...
    for dataset in all_results["datasets"]:
        ...
        if isinstance(mu_max_val, (int, float)) and isinstance(gamma_val, (int, float)):
            print(
                f"{dataset['date']:<12} {dataset['light_intensity']:<5} {mu_max_val:<8.4f} {gamma_val / 1e6:<10.2f} {r2_val:<6.4f} {len(dataset['conditions']):<10}"
            )
        else:
            print(
                f"{dataset['date']:<12} {dataset['light_intensity']:<5} {'N/A':<8} {'N/A':<10} {'N/A':<6} {len(dataset['conditions']):<10}"
            )
