...
def fit_monod_light_parameters(light_intensities, growth_rates):
    ...
    try:
        ...
        popt, pcov = curve_fit(monod_light_model, light_intensities, growth_rates, 
...
def fit_offset_monod_light_parameters(light_intensities, growth_rates):
    ...
    try:
        ...
        popt, pcov = curve_fit(offset_monod_light_model, light_intensities, growth_rates, 
                              p0=p0, bounds=bounds, maxfev=10000)
...
def calculate_steady_state(params, light_intensity, c0_factor=1.0, model=Model.MORTALITYv2, time_max=1000):
...
def steele_light_model(light_intensity, mu_max_steele, K_steele, beta):
    ...
    return mu_max_steele * (light_intensity / (K_steele + light_intensity)) * np.exp(-beta * light_intensity)
...
def steele_gamma_model(light_intensity, gamma_max_steele, K_gamma_steele, beta_gamma):
    ...
    return gamma_max_steele * (light_intensity / (K_gamma_steele + light_intensity)) * np.exp(-beta_gamma * light_intensity)
...
def fit_gamma_monod_parameters(light_intensities, steady_states):
    ...
    try:
        ...
        popt, pcov = curve_fit(gamma_monod_model, light_intensities, steady_states,
                              p0=p0, bounds=bounds, maxfev=10000)
...
def fit_steele_light_parameters(light_intensities, growth_rates):
    ...
    try:
        ...
        popt, pcov = curve_fit(steele_light_model, light_intensities, growth_rates,
...
def fit_steele_gamma_parameters(light_intensities, steady_states):
    ...
    try:
        ...
        popt, pcov = curve_fit(steele_gamma_model, light_intensities, steady_states,
                              p0=p0, bounds=bounds, maxfev=20000)
    ...
    for dataset in data['datasets']:
        ...
        if fitted.get('mu_max') is not None and fitted.get('r_squared', 0) > 0.0:
            ...
            for condition in dataset['conditions']:
                ...
                dataset_info['growth_rates_by_condition'][c0_factor] = condition['growth_rate']

            # Keep the C0*1.0 for backward compatibility
            dataset_info['mu_at_c0_1'] = dataset_info['growth_rates_by_condition'].get(1.0, dataset_info['mu_max'])
            ...
            for condition_name, c0_factor in condition_mapping.items():
                if condition_name in steady_states:
                    dataset_info['steady_states_by_condition'][c0_factor] = steady_states[condition_name]
...
def plot_light_dependency(datasets, output_dir="results"):
    ...
    if np.any(good_fits):
        ax1.errorbar(light_intensities[good_fits], mu_max_values[good_fits] / normalization_factor, 
                    yerr=mu_max_errors[good_fits] / normalization_factor, fmt='o', color='blue', 
                    capsize=3, capthick=2, markersize=8, alpha=0.7, 
                    label='Good fits (R² > 0.8)')
    if np.any(anomalous):
        ax1.errorbar(light_intensities[anomalous], mu_max_values[anomalous] / normalization_factor, 
                    yerr=mu_max_errors[anomalous] / normalization_factor, fmt='x', color='red', 
                    capsize=3, capthick=2, markersize=10, alpha=0.7, 
                    label='Poor fits (R² ≤ 0.8)')
    
    # Add dataset labels
    for i, (x, y, date, r2) in enumerate(zip(light_intensities, mu_max_values / normalization_factor, dates, r_squared_values)):
        color = 'blue' if r2 > 0.8 else 'red'
        ax1.annotate(date, (x, y), xytext=(5, 5), textcoords='offset points', 
    ...
    if np.sum(good_fits) > 1:
        ...
        z = np.polyfit(light_intensities[good_fits], mu_max_values[good_fits], 1)
        p = np.poly1d(z)
        ax1.plot(light_range, p(light_range) / normalization_factor, 'b--', alpha=0.7, 
        ...
        mu_offset, mu_max_light_offset, K_L_offset, r2_offset_monod, offset_param_errors = fit_offset_monod_light_parameters(
            light_intensities[good_fits], mu_max_values[good_fits])

        # Steele light fit
        mu_max_steele, K_steele, beta_steele, r2_steele, steele_param_errors = fit_steele_light_parameters(
            light_intensities[good_fits], mu_max_values[good_fits])

        if mu_max_light is not None:
            monod_curve = monod_light_model(light_range, mu_max_light, K_L)
            ax1.plot(light_range, monod_curve / normalization_factor, 'g-', alpha=0.7, linewidth=2,
                     label=f'Monod: normalized')

        if mu_offset is not None:
            offset_monod_curve = offset_monod_light_model(light_range, mu_offset, mu_max_light_offset, K_L_offset)
            ax1.plot(light_range, offset_monod_curve / normalization_factor, 'm-', alpha=0.7, linewidth=2,
                     label=f'Offset Monod: normalized')

        if mu_max_steele is not None:
            steele_curve = steele_light_model(light_range, mu_max_steele, K_steele, beta_steele)
            ax1.plot(light_range, steele_curve / normalization_factor, 'r-', alpha=0.8, linewidth=2,
        ...
        ax1.text(0.05, 0.95, text_info, transform=ax1.transAxes, fontsize=9,
                bbox=dict(boxstyle="round,pad=0.3", facecolor='lightblue', alpha=0.7))
        
        if mu_max_light is None and mu_offset is None:
            ax1.text(0.05, 0.95, f'Linear r = {corr:.4f}', transform=ax1.transAxes, 
                    bbox=dict(boxstyle="round,pad=0.3", facecolor='lightblue', alpha=0.7))
    
    ax1.set_xlabel('Light Intensity (μmol/m²/s)')
    ax1.set_ylabel('μ_max (normalized to linear fit at L₀=300)')
    ax1.set_title('Maximum Growth Rate vs Light Intensity (Normalized)\n(Fitted parameter from Monod model)')
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=8)
    
    # Plot 2: Γ vs Light Intensity with Monod fitting
    if np.any(good_fits):
        ax2.errorbar(light_intensities[good_fits], gamma_values[good_fits]/1e6,
                    yerr=gamma_errors[good_fits]/1e6, fmt='o', color='blue',
                    capsize=3, capthick=2, markersize=8, alpha=0.7,
                    label='Good fits')
    if np.any(anomalous):
        ax2.errorbar(light_intensities[anomalous], gamma_values[anomalous]/1e6,
                    yerr=gamma_errors[anomalous]/1e6, fmt='x', color='red',
                    capsize=3, capthick=2, markersize=10, alpha=0.7,
                    label='Poor fits')
    ...
    if np.sum(good_fits) > 1:
        # Monod fit
        gamma_max_fitted, K_gamma_fitted, r2_gamma_fitted, _ = fit_gamma_monod_parameters(
            light_intensities[good_fits], gamma_values[good_fits])

        # Steele fit
        gamma_max_steele_fitted, K_gamma_steele_fitted, beta_gamma_fitted, r2_gamma_steele_fitted, _ = fit_steele_gamma_parameters(
            light_intensities[good_fits], gamma_values[good_fits])

        if gamma_max_fitted is not None:
            gamma_curve_fitted = gamma_monod_model(light_range, gamma_max_fitted, K_gamma_fitted)
            ax2.plot(light_range, gamma_curve_fitted/1e6, 'g-', alpha=0.8, linewidth=2,
                     label=f'Γ Monod (R² = {r2_gamma_fitted:.3f})')

        if gamma_max_steele_fitted is not None:
            gamma_steele_curve_fitted = steele_gamma_model(light_range, gamma_max_steele_fitted, K_gamma_steele_fitted, beta_gamma_fitted)
            ax2.plot(light_range, gamma_steele_curve_fitted/1e6, 'r-', alpha=0.8, linewidth=2,
                     label=f'Γ Steele (R² = {r2_gamma_steele_fitted:.3f})')

        # Display fit quality and parameters (prefer Steele if better)
        if gamma_max_steele_fitted is not None and r2_gamma_steele_fitted > r2_gamma_fitted:
            ax2.text(0.05, 0.95, f'Best: Steele Model\nR² = {r2_gamma_steele_fitted:.3f}\nβ = {beta_gamma_fitted:.4f}',
                    transform=ax2.transAxes, fontsize=9,
                    bbox=dict(boxstyle="round,pad=0.3", facecolor='lightcoral', alpha=0.7))
        elif gamma_max_fitted is not None:
            ax2.text(0.05, 0.95, f'Best: Monod Model\nR² = {r2_gamma_fitted:.3f}',
                    transform=ax2.transAxes, fontsize=9,
                    bbox=dict(boxstyle="round,pad=0.3", facecolor='lightgreen', alpha=0.7))

    # Add dataset labels
    for i, (x, y, date, r2) in enumerate(zip(light_intensities, gamma_values/1e6, dates, r_squared_values)):
        color = 'blue' if r2 > 0.8 else 'red'
        ax2.annotate(date, (x, y), xytext=(5, 5), textcoords='offset points',
                    fontsize=8, alpha=0.8, color=color)

    ax2.set_xlabel('Light Intensity (μmol/m²/s)')
    ax2.set_ylabel('Γ (×10⁶ cells/mL)')
    ax2.set_title('Half-Saturation Constant vs Light Intensity\n(Fitted parameter Γ with potential Monod dependency)')
    ax2.grid(True, alpha=0.3)
    ax2.legend(fontsize=8)
    
    # Plot 3: R² vs Light Intensity (fit quality)
    colors = ['red' if r < 0.8 else 'blue' for r in r_squared_values]
    scatter = ax3.scatter(light_intensities, r_squared_values, c=colors, s=80, alpha=0.7)
    
    # Add dataset labels
    for i, (x, y, date) in enumerate(zip(light_intensities, r_squared_values, dates)):
        ax3.annotate(date, (x, y), xytext=(5, 5), textcoords='offset points', 
                    fontsize=8, alpha=0.8)
    
    ax3.axhline(y=0.8, color='gray', linestyle='--', alpha=0.7, label='R² = 0.8 threshold')
    ax3.set_xlabel('Light Intensity (μmol/m²/s)')
    ax3.set_ylabel('R² (Model Fit Quality)')
    ax3.set_title('Monod Model Fit Quality vs Light Intensity\n(How well μ vs C₀ fits Monod model)')
    ax3.set_ylim(0, 1.1)
    ...
    for i, c0_factor in enumerate(c0_conditions):
        ...
        if len(c0_growth_rates) > 0:
            ...
            if np.any(c0_good_fits):
                ax4.scatter(c0_light_intensities[c0_good_fits], c0_growth_rates[c0_good_fits], 
                           c=[colors_c0[i]], s=60, alpha=0.8, label=f'C0*{c0_factor}')
            if np.any(~c0_good_fits):
                ax4.scatter(c0_light_intensities[~c0_good_fits], c0_growth_rates[~c0_good_fits], 
                           c=[colors_c0[i]], s=60, alpha=0.8, marker='x')
            
            # Fit both linear and Monod models for good data points
            if np.sum(c0_good_fits) > 1:
                # Linear fit
                z_c0 = np.polyfit(c0_light_intensities[c0_good_fits], c0_growth_rates[c0_good_fits], 1)
                p_c0 = np.poly1d(z_c0)
                ax4.plot(light_range, p_c0(light_range), '--', alpha=0.5, color=colors_c0[i],
                         linewidth=1.5, label=f'C0*{c0_factor} linear' if i < 2 else "")
                ...
                # Offset Monod light fit
                mu_offset_c0, mu_max_light_offset_c0, K_L_offset_c0, r2_offset_monod_c0, _ = fit_offset_monod_light_parameters(
                    c0_light_intensities[c0_good_fits], c0_growth_rates[c0_good_fits])
                
                if mu_max_light_c0 is not None and r2_monod_c0 > 0.5:  # Only show if reasonable fit
                    monod_curve_c0 = monod_light_model(light_range, mu_max_light_c0, K_L_c0)
                    ax4.plot(light_range, monod_curve_c0, '-', alpha=0.7, color=colors_c0[i],
                             linewidth=2, label=f'C0*{c0_factor} Monod' if i < 2 else "")
                
                if mu_offset_c0 is not None and r2_offset_monod_c0 > 0.5:  # Only show if reasonable fit
                    offset_monod_curve_c0 = offset_monod_light_model(light_range, mu_offset_c0, mu_max_light_offset_c0, K_L_offset_c0)
                    ax4.plot(light_range, offset_monod_curve_c0, ':', alpha=0.8, color=colors_c0[i],
                             linewidth=2.5, label=f'C0*{c0_factor} Offset Monod' if i < 2 else "")
    
    ax4.set_xlabel('Light Intensity (μmol/m²/s)')
    ax4.set_ylabel('Growth Rate (h⁻¹)')
    ax4.set_title('Growth Rates for All C0 Conditions vs Light Intensity\n(Experimental μ values for each dilution)')
...
def load_fitted_parameters_from_tuning(model_dir="Model.MORTALITYv2", param_file="erlen.yaml"):
...
def calculate_simulated_growth_rate(params, light_intensity, c0_factor, N_initial=None, model=Model.MORTALITYv2):
    ...
    try:
        ...
        print(f"Error calculating growth rate for L0={light_intensity}, C0*{c0_factor}: {e}")
        return None

def generate_simulated_growth_rates(fitted_params, light_intensities, c0_factors, model=Model.MORTALITYv2):
    ...
    for c0_factor in c0_factors:
        ...
        for light in light_intensities:
            ...
            growth_rate = calculate_simulated_growth_rate(fitted_params, light, c0_factor, model=model)
...
def generate_simulated_steady_states(fitted_params, light_intensities, c0_factors, model=Model.MORTALITYv2):
    ...
    for c0_factor in c0_factors:
        ...
        for light in light_intensities:
            steady_state = calculate_steady_state(fitted_params, light, c0_factor, model=model)
            if steady_state is not None:
                steady_states.append(steady_state)
                valid_lights.append(light)
            else:
                print(f"Failed to calculate steady state for L0={light}, C0*{c0_factor}")
...
def plot_steady_states(datasets, output_dir="results"):
    ...
    if fitted_params is not None:
        ...
        simulated_data = generate_simulated_steady_states(fitted_params, light_range, c0_conditions)
    ...
    for i, c0_factor in enumerate(c0_conditions):
        ...
        for j, dataset in enumerate(datasets):
            if c0_factor in dataset.get('steady_states_by_condition', {}):
                c0_steady_states.append(dataset['steady_states_by_condition'][c0_factor])
                c0_light_intensities.append(dataset['light_intensity'])
                c0_good_fits.append(dataset['r_squared'] > 0.8)
        ...
        if len(c0_steady_states) > 0:
            # Plot good and bad fits with different markers
            if np.any(c0_good_fits):
                ax1.scatter(c0_light_intensities[c0_good_fits], c0_steady_states[c0_good_fits]/1e6,
                           c=[colors_c0[i]], s=60, alpha=0.8, label=f'C0*{c0_factor}')
            if np.any(~c0_good_fits):
                ax1.scatter(c0_light_intensities[~c0_good_fits], c0_steady_states[~c0_good_fits]/1e6,
                           c=[colors_c0[i]], s=60, alpha=0.8, marker='x')
            ...
            if np.sum(c0_good_fits) > 1:
                ...
                # Steele fit
                gamma_max_steele_c0, K_gamma_steele_c0, beta_gamma_c0, r2_gamma_steele_c0, _ = fit_steele_gamma_parameters(
                    c0_light_intensities[c0_good_fits], c0_steady_states[c0_good_fits])

                # Choose best fit and plot it
                best_is_steele = (gamma_max_steele_c0 is not None and
                                r2_gamma_steele_c0 > r2_gamma_c0 and
                                r2_gamma_steele_c0 > 0.3)

                if best_is_steele:
                    gamma_steele_curve_c0 = steele_gamma_model(light_range, gamma_max_steele_c0, K_gamma_steele_c0, beta_gamma_c0)
                    ax1.plot(light_range, gamma_steele_curve_c0/1e6, '-', alpha=0.7, color=colors_c0[i],
                             linewidth=2, label=f'C0*{c0_factor} Steele' if c0_factor == 1.0 else "")
                    best_r2 = r2_gamma_steele_c0
                    model_type = "Steele"
                elif gamma_max_c0 is not None and r2_gamma_c0 > 0.3:
                    gamma_curve_c0 = gamma_monod_model(light_range, gamma_max_c0, K_gamma_c0)
                    ax1.plot(light_range, gamma_curve_c0/1e6, '-', alpha=0.7, color=colors_c0[i],
                             linewidth=2, label=f'C0*{c0_factor} Monod' if c0_factor == 1.0 else "")
                    best_r2 = r2_gamma_c0
                    model_type = "Monod"
                else:
                    best_r2 = 0
                ...
                if c0_factor == 1.0 and best_r2 > 0.3:
                    ax1.text(0.6, 0.95, f'C0*{c0_factor}: {model_type}\nR² = {best_r2:.3f}',
                            transform=ax1.transAxes, fontsize=8,
                            bbox=dict(boxstyle="round,pad=0.2", facecolor=colors_c0[i], alpha=0.3))
        ...
        if simulated_data is not None and c0_factor in simulated_data:
            ...
            ax1.plot(sim_data['light_intensities'], sim_data['steady_states']/1e6, '--',
                    alpha=0.8, color=colors_c0[i], linewidth=2,
                    label=f'Simulated C0*{c0_factor}' if c0_factor == 1.0 else "")

    ax1.set_xlabel('Light Intensity (μmol/m²/s)')
    ax1.set_ylabel('Steady State Cell Density (×10⁶ cells/mL)')
    ax1.set_title('Experimental vs Simulated Steady States vs Light Intensity\n(All C0 conditions - dashed lines: tuning_ODE.py simulations)')
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=8)
    ...
    if len(steady_states_c0_1) > 0:
        ...
        if np.any(good_fits_ss):
            ax2.scatter(light_intensities_ss[good_fits_ss], steady_states_c0_1[good_fits_ss]/1e6,
                       color='blue', s=80, alpha=0.7, label='Good fits (R² > 0.8)')
        if np.any(~good_fits_ss):
            ax2.scatter(light_intensities_ss[~good_fits_ss], steady_states_c0_1[~good_fits_ss]/1e6,
                       color='red', s=80, alpha=0.7, marker='x', label='Poor fits (R² ≤ 0.8)')

        # Fit both Γ_eff Monod and Steele models for good fits
        if np.sum(good_fits_ss) > 1:
            # Monod fit
            gamma_max, K_gamma, r2_gamma, param_errors_gamma = fit_gamma_monod_parameters(
                light_intensities_ss[good_fits_ss], steady_states_c0_1[good_fits_ss])

            # Steele fit
            gamma_max_steele, K_gamma_steele, beta_gamma_steele, r2_gamma_steele, param_errors_steele = fit_steele_gamma_parameters(
                light_intensities_ss[good_fits_ss], steady_states_c0_1[good_fits_ss])

            if gamma_max is not None:
                gamma_curve = gamma_monod_model(light_range, gamma_max, K_gamma)
                ax2.plot(light_range, gamma_curve/1e6, 'g-', alpha=0.8, linewidth=2,
                         label=f'Monod: R² = {r2_gamma:.3f}')

            if gamma_max_steele is not None:
                gamma_steele_curve = steele_gamma_model(light_range, gamma_max_steele, K_gamma_steele, beta_gamma_steele)
                ax2.plot(light_range, gamma_steele_curve/1e6, 'r-', alpha=0.8, linewidth=2,
            ...
            if best_is_steele:
                ...
                print(f"Gamma_max = {gamma_max_steele:.0f} cells/mL  ({gamma_max_steele/1e6:.1f}×10⁶)")
                ...
                print(f"Equation: Gamma_eff = {gamma_max_steele:.0f} * (L0 / ({K_gamma_steele:.0f} + L0)) * exp(-{beta_gamma_steele:.6f} * L0)")
                ...
                print(f"Gamma_max = {gamma_max:.0f} cells/mL  ({gamma_max/1e6:.1f}×10⁶)")
                print(f"K_gamma = {K_gamma:.0f} μmol/m²/s")
                print(f"R² = {r2_gamma:.3f}")
                print(f"Equation: Gamma_eff = {gamma_max:.0f} * L0 / ({K_gamma:.0f} + L0)")
        ...
        if simulated_data is not None and 1.0 in simulated_data:
            ...
            ax2.plot(sim_data['light_intensities'], sim_data['steady_states']/1e6, 'k--',
                    alpha=0.8, linewidth=3, label='Simulated (tuning_ODE.py)')

        # Add dataset labels
        for i, (x, y, date, r2) in enumerate(zip(light_intensities_ss, steady_states_c0_1/1e6,
                                                 [d['date'] for d in datasets if 1.0 in d.get('steady_states_by_condition', {})],
                                                 good_fits_ss)):
    ...
    ax2.set_title('C0*1.0: Experimental vs Simulated Steady States\n(Black dashed: tuning_ODE.py simulation)')
    ax2.grid(True, alpha=0.3)
    ax2.legend(fontsize=8)
...
def plot_growth_rates(datasets, output_dir="results"):
    ...
    if fitted_params is not None:
        ...
        simulated_growth_data = generate_simulated_growth_rates(fitted_params, light_range, c0_conditions)
        print(f"Generated simulated growth rate data for {len(simulated_growth_data)} C0 conditions")
    ...
    for i, c0_factor in enumerate(c0_conditions):
        ...
        if len(c0_growth_rates) > 0:
            # Plot good and bad fits with different markers
            if np.any(c0_good_fits):
                ax1.scatter(c0_light_intensities[c0_good_fits], c0_growth_rates[c0_good_fits],
                           c=[colors_c0[i]], s=60, alpha=0.8, label=f'C0*{c0_factor}')
            if np.any(~c0_good_fits):
                ax1.scatter(c0_light_intensities[~c0_good_fits], c0_growth_rates[~c0_good_fits],
                           c=[colors_c0[i]], s=60, alpha=0.8, marker='x')
            ...
            if np.sum(c0_good_fits) > 1:
                ...
                mu_max_steele_c0, K_steele_c0, beta_steele_c0, r2_steele_c0, _ = fit_steele_light_parameters(
                    c0_light_intensities[c0_good_fits], c0_growth_rates[c0_good_fits])

                if mu_max_steele_c0 is not None and r2_steele_c0 > 0.3:
                    steele_curve_c0 = steele_light_model(light_range, mu_max_steele_c0, K_steele_c0, beta_steele_c0)
                    ax1.plot(light_range, steele_curve_c0, '-', alpha=0.7, color=colors_c0[i],
                             linewidth=2, label=f'C0*{c0_factor} Steele' if c0_factor == 1.0 else "")

        # Add simulated data for this C0 condition if available
        if simulated_growth_data is not None and c0_factor in simulated_growth_data:
            sim_data = simulated_growth_data[c0_factor]
            ax1.plot(sim_data['light_intensities'], sim_data['growth_rates'], '--',
                    alpha=0.8, color=colors_c0[i], linewidth=2,
                    label=f'Simulated C0*{c0_factor}' if c0_factor == 1.0 else "")

    ax1.set_xlabel('Light Intensity (μmol/m²/s)')
    ax1.set_ylabel('Specific Growth Rate (h⁻¹)')
    ax1.set_title('Experimental vs Simulated Growth Rates vs Light Intensity\n(All C0 conditions - dashed lines: tuning_ODE.py simulations)')
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=8)
    ...
    if len(growth_rates_c0_1) > 0:
        ...
        if np.any(good_fits_gr):
            ax2.scatter(light_intensities_gr[good_fits_gr], growth_rates_c0_1[good_fits_gr],
                       color='blue', s=80, alpha=0.7, label='Good fits (R² > 0.8)')
        if np.any(~good_fits_gr):
            ax2.scatter(light_intensities_gr[~good_fits_gr], growth_rates_c0_1[~good_fits_gr],
                       color='red', s=80, alpha=0.7, marker='x', label='Poor fits (R² ≤ 0.8)')
        ...
        if np.sum(good_fits_gr) > 1:
            # Steele fit
            mu_max_steele, K_steele, beta_steele, r2_steele, param_errors_steele = fit_steele_light_parameters(
                light_intensities_gr[good_fits_gr], growth_rates_c0_1[good_fits_gr])

            # Monod fit for comparison
            mu_max_monod, K_monod, r2_monod, param_errors_monod = fit_monod_light_parameters(
                light_intensities_gr[good_fits_gr], growth_rates_c0_1[good_fits_gr])

            if mu_max_steele is not None:
                steele_curve = steele_light_model(light_range, mu_max_steele, K_steele, beta_steele)
                ax2.plot(light_range, steele_curve, 'r-', alpha=0.8, linewidth=2,
                         label=f'Steele: R² = {r2_steele:.3f}')

            if mu_max_monod is not None:
                monod_curve = monod_light_model(light_range, mu_max_monod, K_monod)
                ax2.plot(light_range, monod_curve, 'g-', alpha=0.8, linewidth=2,
        ...
        if simulated_growth_data is not None and 1.0 in simulated_growth_data:
            ...
            ax2.plot(sim_data['light_intensities'], sim_data['growth_rates'], 'k--',
                    alpha=0.8, linewidth=3, label='Simulated (tuning_ODE.py)')

        # Add dataset labels
        for i, (x, y, date, r2) in enumerate(zip(light_intensities_gr, growth_rates_c0_1,
                                                 [d['date'] for d in datasets if 1.0 in d.get('growth_rates_by_condition', {})],
                                                 good_fits_gr)):
            color = 'blue' if r2 else 'red'
            ax2.annotate(date, (x, y), xytext=(5, 5), textcoords='offset points',
                        fontsize=8, alpha=0.8, color=color)
    ...
    ax2.set_title('C0*1.0: Experimental vs Simulated Growth Rates\n(Black dashed: tuning_ODE.py simulation)')
    ax2.grid(True, alpha=0.3)
    ax2.legend(fontsize=8)
...
def plot_experimental_data_with_growth_windows(datasets, output_dir="results"):
    ...
    for i, dataset in enumerate(datasets):
        ...
        if not growth_curves:
            ax.text(0.5, 0.5, f'No growth curves\nfound for {date}',
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title(f'{date} (L0={light_intensity} μmol/m²/s)')
            continue
        ...
        for condition in dataset['conditions']:
            ...
            if condition_name in growth_curves:
                ...
                ax.semilogy(time, cells, '-', color=condition_colors[condition_name],
                           alpha=0.7, linewidth=1, label=condition_name)
                ...
                if np.any(exp_mask):
                    ax.semilogy(time[exp_mask], cells[exp_mask], 'o',
                    ...
                    if not np.isnan(growth_rate) and growth_rate > 0:
                        ...
                        if len(t_exp) > 0:
                            ...
                            t_rel = t_exp - t_exp[0]  # relative time from start of exp phase

                            # Calculate fitted line
                            N0_fit = cells_exp[0]
                            fitted_cells = N0_fit * np.exp(growth_rate * t_rel)

                            ax.semilogy(t_exp, fitted_cells, '--',
                                       color=condition_colors[condition_name],
                            ...
                            if c0_factor == 1.0:
                                mid_idx = len(t_exp) // 2
                                ax.annotate(f'μ = {growth_rate:.3f} h⁻¹',
        ...
        ax.text(0.02, 0.98, 'Circles: exponential phase\nDashed: fitted growth rate',
...
def print_summary_table(datasets):
    ...
    print("="*100)
    print(f"{'Dataset':<12} {'L0':<5} {'μ_max':<10} {'Γ(×10⁶)':<12} {'R²':<8} {'μ@C0*1.0':<10} {'Conditions':<10}")
    print("-"*100)
    
    for dataset in sorted(datasets, key=lambda x: x['light_intensity']):
        print(f"{dataset['date']:<12} {dataset['light_intensity']:<5.0f} "
    ...
    if len(good_datasets) > 1:
        ...
        print(f"μ_max vs L₀:     μ_max = {z_mu_max[0]:.2e} × L₀ + {z_mu_max[1]:.4f} (r = {corr_mu_max:.4f})")
        print(f"μ(C0*1.0) vs L₀: μ = {z_mu_c0[0]:.2e} × L₀ + {z_mu_c0[1]:.4f} (r = {corr_mu_c0:.4f})")
        
        # Monod light fits
        print("\nMonod light relationships for good quality data (R² > 0.8):")
        mu_max_light_fit, K_L_fit, r2_monod_fit, _ = fit_monod_light_parameters(light_vals, mu_max_vals)
        if mu_max_light_fit is not None:
            print(f"μ_max vs L₀:     μ_max = {mu_max_light_fit:.4f} × L₀ / ({K_L_fit:.0f} + L₀) (R² = {r2_monod_fit:.4f})")
            
        mu_max_light_c0_fit, K_L_c0_fit, r2_monod_c0_fit, _ = fit_monod_light_parameters(light_vals, mu_c0_vals)
        if mu_max_light_c0_fit is not None:
            print(f"μ(C0*1.0) vs L₀: μ = {mu_max_light_c0_fit:.4f} × L₀ / ({K_L_c0_fit:.0f} + L₀) (R² = {r2_monod_c0_fit:.4f})")
        
        # Offset Monod light fits
        print("\nOffset Monod light relationships for good quality data (R² > 0.8):")
        mu_offset_fit, mu_max_light_offset_fit, K_L_offset_fit, r2_offset_monod_fit, _ = fit_offset_monod_light_parameters(light_vals, mu_max_vals)
        if mu_offset_fit is not None:
            print(f"μ_max vs L₀:     μ = {mu_offset_fit:.4f} + {mu_max_light_offset_fit:.4f} × L₀ / ({K_L_offset_fit:.0f} + L₀) (R² = {r2_offset_monod_fit:.4f})")
            
        mu_offset_c0_fit, mu_max_light_offset_c0_fit, K_L_offset_c0_fit, r2_offset_monod_c0_fit, _ = fit_offset_monod_light_parameters(light_vals, mu_c0_vals)
        if mu_offset_c0_fit is not None:
            print(f"μ(C0*1.0) vs L₀: μ = {mu_offset_c0_fit:.4f} + {mu_max_light_offset_c0_fit:.4f} × L₀ / ({K_L_offset_c0_fit:.0f} + L₀) (R² = {r2_offset_monod_c0_fit:.4f})")
        ...
        for c0_factor in c0_conditions:
            ...
            if len(c0_growth_rates) > 1:
                ...
                print(f"μ(C0*{c0_factor:<4}) vs L₀: μ = {z_c0[0]:.2e} × L₀ + {z_c0[1]:.4f} (r = {corr_c0:.4f})")
        ...
        for c0_factor in c0_conditions:
            ...
            if len(c0_growth_rates) > 1:
                ...
                mu_max_light_c0, K_L_c0, r2_monod_c0, _ = fit_monod_light_parameters(c0_light_vals, c0_mu_vals)
                
                if mu_max_light_c0 is not None:
                    print(f"μ(C0*{c0_factor:<4}) vs L₀: μ = {mu_max_light_c0:.4f} × L₀ / ({K_L_c0:.0f} + L₀) (R² = {r2_monod_c0:.4f})")
        ...
        for c0_factor in c0_conditions:
            ...
            if len(c0_growth_rates) > 1:
                ...
                mu_offset_c0, mu_max_light_offset_c0, K_L_offset_c0, r2_offset_monod_c0, _ = fit_offset_monod_light_parameters(c0_light_vals, c0_mu_vals)
                
                if mu_offset_c0 is not None:
                    print(f"μ(C0*{c0_factor:<4}) vs L₀: μ = {mu_offset_c0:.4f} + {mu_max_light_offset_c0:.4f} × L₀ / ({K_L_offset_c0:.0f} + L₀) (R² = {r2_offset_monod_c0:.4f})")
    ...
    if anomalous_datasets:
        ...
        for d in anomalous_datasets:
            print(f"  {d['date']} (L₀ = {d['light_intensity']} μmol/m²/s): R² = {d['r_squared']:.4f}")
...
def main():
    ...
    print(f"Experimental data with growth windows plot saved to: {experimental_data_path}")
