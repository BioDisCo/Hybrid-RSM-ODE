...
def calculate_model_growth_rate(
    params, light_intensity, c0_factor, N_initial=None, time_max=50
):
    ...
    try:
        ...
        sol = solve_ivp(
            fun=lambda t, y: dxdt_algae_model(
                t, y, adjusted_params, model=Model.TWO_POP_MODEL
            ),
            t_span=(0, time_max),
            y0=y0,
            method="LSODA",
...
def analyze_single_dataset_with_model(
    filepath: str, light_intensity: int, date: str, params: AlgalSysParameters
) -> Dict:
    ...
    try:
        ...
        for condition, (time, od) in growth_data.items():
            ...
            mu_exp = calculate_specific_growth_rate(
                time, cells_per_ml, (exp_start, exp_end)
            )
            ...
            steady_state_model = calculate_model_steady_state(
                params, light_intensity, c0_factor
            )

            if not np.isnan(mu_exp):
                results["experimental_growth_rates"][condition] = float(mu_exp)
                results["model_growth_rates"][condition] = (
                    float(mu_model) if not np.isnan(mu_model) else None
                )
                results["experimental_steady_states"][condition] = float(
                    steady_state_exp
                )
                results["model_steady_states"][condition] = (
                    float(steady_state_model)
                    if not np.isnan(steady_state_model)
                    else None
                )

                print(f"  Experimental μ: {mu_exp:.4f} h⁻¹")
                print(
                    f"  Model μ:        {mu_model:.4f} h⁻¹"
                    if not np.isnan(mu_model)
                    else "  Model μ:        N/A"
                )
                print(f"  Exp. steady state:   {steady_state_exp:.2e} cells/mL")
                print(
                    f"  Model steady state:  {steady_state_model:.2e} cells/mL"
                    if not np.isnan(steady_state_model)
                    else "  Model steady state:  N/A"
                )
                ...
                results["conditions"].append(
                    {
                        ...
                        "model_growth_rate": float(mu_model)
                        if not np.isnan(mu_model)
                        else None,
                        "exp_steady_state": float(steady_state_exp),
                        "model_steady_state": float(steady_state_model)
                        if not np.isnan(steady_state_model)
                        else None,
        ...
        generate_growth_rate_plots(
            filepath, date, light_intensity, growth_data, results, params
        )
...
def generate_growth_rate_plots(
    filepath, date, light_intensity, growth_data, results, params
):
    ...
    conditions = sorted(
        results["experimental_growth_rates"].keys(),
    ...
    model_growth_rates = [
        results["model_growth_rates"][c]
        if results["model_growth_rates"][c] is not None
        else np.nan
        for c in conditions
    ]
    ...
    model_steady_states = [
        results["model_steady_states"][c]
        if results["model_steady_states"][c] is not None
        else np.nan
        for c in conditions
    ]
    ...
    for i, (condition, (time, od)) in enumerate(
        ...
        cond_result = next(
            (c for c in results["conditions"] if c["condition"] == condition), None
        )
        if cond_result:
            exp_start = cond_result["exp_start"]
            exp_end = cond_result["exp_end"]
            ax.axvspan(
                exp_start, exp_end, alpha=0.2, color="green", label="Exponential phase"
            )
    ...
    plt.suptitle(
        f"Growth Analysis: {date} (L0={light_intensity} µmol/m²/s)",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
    ...
    if np.any(valid_model):
        ax.plot(
            np.array(c0_factors)[valid_model],
            np.array(model_growth_rates)[valid_model],
            "^--",
            color="#A23B72",
            linewidth=2.5,
            markersize=10,
    ...
    ax.set_xlabel(
        "Normalized nutrient concentration (C₀)", fontsize=14, fontweight="bold"
    )
    ax.set_ylabel("Specific growth rate μ (h⁻¹)", fontsize=14, fontweight="bold")
    ax.set_title(
        f"Growth Rate vs Nutrient Concentration\n{date} (L0={light_intensity} µmol/m²/s)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
    ...
    ax.plot(
        c0_factors,
        np.array(exp_steady_states) / 1e6,
        "o-",
        color="#2E86AB",
        linewidth=2.5,
        markersize=10,
    ...
    if np.any(valid_model):
        ax.plot(
            np.array(c0_factors)[valid_model],
            np.array(model_steady_states)[valid_model] / 1e6,
            "^--",
            color="#A23B72",
            linewidth=2.5,
            markersize=10,
    ...
    ax.set_title(
        f"Steady State vs Nutrient Concentration\n{date} (L0={light_intensity} µmol/m²/s)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def plot_light_dependency_comparison(all_results, params):
    ...
    for result in all_results:
        if "C0*1.0" in result["experimental_growth_rates"]:
            ...
            c0_1_model_steady.append(
                result["model_steady_states"].get("C0*1.0", np.nan)
            )
    ...
    ax1.plot(
        light_intensities[valid_exp],
        c0_1_exp_growth[valid_exp],
        "o-",
        color="#2E86AB",
        linewidth=3,
        markersize=12,
        label="Experimental (C0*1.0)",
        markeredgecolor="white",
        markeredgewidth=2,
    )
    ...
    if np.any(valid_model):
        ax1.plot(
            light_intensities[valid_model],
            c0_1_model_growth[valid_model],
            "^--",
            color="#A23B72",
            linewidth=3,
            markersize=12,
            label="TWO_POP_MODEL (C0*1.0)",
            markeredgecolor="white",
            markeredgewidth=2,
        )
    ...
    # Add date labels
    for x, y, date in zip(
        light_intensities[valid_exp], c0_1_exp_growth[valid_exp], dates[valid_exp]
    ):
        ax1.annotate(
            date,
            (x, y),
    ...
    ax1.set_title(
        "Growth Rate vs Light Intensity (C0*1.0)",
        fontsize=16,
        fontweight="bold",
        pad=15,
    )
    ...
    if np.any(valid_model):
        ax2.plot(
            light_intensities[valid_model],
            c0_1_model_steady[valid_model] / 1e6,
            "^--",
            color="#A23B72",
            linewidth=3,
            markersize=12,
    ...
    # Add date labels
    for x, y, date in zip(
        light_intensities[valid_exp], c0_1_exp_steady[valid_exp] / 1e6, dates[valid_exp]
    ):
        ax2.annotate(
            date,
            (x, y),
    ...
    ax2.set_title(
        "Steady State vs Light Intensity (C0*1.0)",
        fontsize=16,
        fontweight="bold",
        pad=15,
    )
    ...
    plt.suptitle(
        "Light Dependency Analysis - TWO_POP_MODEL",
        fontsize=18,
        fontweight="bold",
        y=1.02,
    )
...
def main():
    ...
    files_to_analyze = [
        {
            "filepath": "data_exp_Chlamy_16-09-24.csv",
            "light_intensity": 22,
            "date": "16-09-24",
        },
        {
            "filepath": "data_exp_Chlamy_21-10-24.csv",
            "light_intensity": 45,
            "date": "21-10-24",
        },
        {
            "filepath": "data_exp_Chlamy_04-11-24.csv",
            "light_intensity": 45,
            "date": "04-11-24",
        },
        {
            "filepath": "data_exp_Chlamy_17-02-25.csv",
            "light_intensity": 90,
            "date": "17-02-25",
        },
        {
            "filepath": "data_exp_Chlamy_01-07-25.csv",
            "light_intensity": 180,
            "date": "01-07-25",
        },
        {
            "filepath": "data_exp_Chlamy_07-07-25.csv",
            "light_intensity": 300,
            "date": "07-07-25",
        },
    ]
    ...
    for result in all_results:
        ...
        diff = (
            exp_mu - model_mu
            if not np.isnan(exp_mu) and not np.isnan(model_mu)
            else np.nan
        )

        if not np.isnan(exp_mu):
            print(
                f"{result['date']:<12} {result['light_intensity']:<5} "
                f"{exp_mu:<15.4f} {model_mu:<12.4f} {diff:<10.4f}"
            )
        else:
            print(
                f"{result['date']:<12} {result['light_intensity']:<5} {'N/A':<15} {'N/A':<12} {'N/A':<10}"
            )
