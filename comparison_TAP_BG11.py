...
def load_and_prepare_data(filepath, medium_name):
    ...
    # List of replicates
    replicates = ['0A', '0B', '0C', '1A', '1B', '1C', '2A', '2B', '2C',
                  '3A', '3B', '3C', '4A', '4B', '4C']
    ...
    print(f"[{medium_name}] Data loaded: {len(data_df)} time points, "
...
def compute_cld(mu_dict, alpha=0.05):
    ...
    sig = [[tukey.pvalue[order[i]][order[j]] < alpha
    ...
    return {
        ...
        'h_stat': h_stat, 'p_kruskal': p_kruskal,
        'letters': letters, 'tukey': tukey
    }
...
if __name__ == "__main__":
    ...
    mu_bg11_51, r2_bg11_51 = calculate_all_mu(
        data_bg11_51,
        EXP_PHASE_BG11_51['start'],
        EXP_PHASE_BG11_51['end']
    )
    ...
    mu_tap_51, r2_tap_51 = calculate_all_mu(
        data_tap_51,
        EXP_PHASE_TAP_51['start'],
        EXP_PHASE_TAP_51['end']
    )
    ...
    print(f"Exponential phase TAP: {EXP_PHASE_TAP_51['start']}-{EXP_PHASE_TAP_51['end']}h")
    ...
    for i, cond in enumerate(['0', '1', '2', '3', '4']):
        ...
        print(f"  {COND_NAMES[i]}: μmax = {mean_mu:.5f} ± {std_mu:.5f} h⁻¹ "
    ...
    mu_bg11_102, r2_bg11_102 = calculate_all_mu(
        data_bg11_102,
        EXP_PHASE_BG11_102['start'],
        EXP_PHASE_BG11_102['end']
    )
    ...
    mu_tap_102, r2_tap_102 = calculate_all_mu(
        data_tap_102,
        EXP_PHASE_TAP_102['start'],
        EXP_PHASE_TAP_102['end']
    )
    ...
    print(f"Exponential phase TAP: {EXP_PHASE_TAP_102['start']}-{EXP_PHASE_TAP_102['end']}h")
    ...
    sig_b = "***" if cld_bg11_51['p_anova'] < 0.001 else "**" if cld_bg11_51['p_anova'] < 0.01 else "*" if cld_bg11_51['p_anova'] < 0.05 else "ns"
    sig_t = "***" if cld_tap_51['p_anova'] < 0.001 else "**" if cld_tap_51['p_anova'] < 0.01 else "*" if cld_tap_51['p_anova'] < 0.05 else "ns"
    print(f"  BG11: F(4,10) = {cld_bg11_51['f_stat']:.3f}, p = {cld_bg11_51['p_anova']:.6f} ({sig_b})")
    print(f"  TAP:  F(4,10) = {cld_tap_51['f_stat']:.3f}, p = {cld_tap_51['p_anova']:.6f} ({sig_t})")

    print(f"\n  Kruskal-Wallis (non-parametric check):")
    sig_bk = "***" if cld_bg11_51['p_kruskal'] < 0.001 else "**" if cld_bg11_51['p_kruskal'] < 0.01 else "*" if cld_bg11_51['p_kruskal'] < 0.05 else "ns"
    sig_tk = "***" if cld_tap_51['p_kruskal'] < 0.001 else "**" if cld_tap_51['p_kruskal'] < 0.01 else "*" if cld_tap_51['p_kruskal'] < 0.05 else "ns"
    print(f"  BG11: H = {cld_bg11_51['h_stat']:.3f}, p = {cld_bg11_51['p_kruskal']:.6f} ({sig_bk})")
    print(f"  TAP:  H = {cld_tap_51['h_stat']:.3f}, p = {cld_tap_51['p_kruskal']:.6f} ({sig_tk})")

    print(f"\n  Tukey HSD Compact Letter Display (51 µmol/m²/s):")
    for i in range(5):
        print(f"    {COND_NAMES[i]}: BG11={cld_bg11_51['letters'][i]}  TAP={cld_tap_51['letters'][i]}")
    ...
    sig_b = "***" if cld_bg11_102['p_anova'] < 0.001 else "**" if cld_bg11_102['p_anova'] < 0.01 else "*" if cld_bg11_102['p_anova'] < 0.05 else "ns"
    sig_t = "***" if cld_tap_102['p_anova'] < 0.001 else "**" if cld_tap_102['p_anova'] < 0.01 else "*" if cld_tap_102['p_anova'] < 0.05 else "ns"
    print(f"  BG11: F(4,10) = {cld_bg11_102['f_stat']:.3f}, p = {cld_bg11_102['p_anova']:.6f} ({sig_b})")
    print(f"  TAP:  F(4,10) = {cld_tap_102['f_stat']:.3f}, p = {cld_tap_102['p_anova']:.6f} ({sig_t})")

    print(f"\n  Kruskal-Wallis (non-parametric check):")
    sig_bk = "***" if cld_bg11_102['p_kruskal'] < 0.001 else "**" if cld_bg11_102['p_kruskal'] < 0.01 else "*" if cld_bg11_102['p_kruskal'] < 0.05 else "ns"
    sig_tk = "***" if cld_tap_102['p_kruskal'] < 0.001 else "**" if cld_tap_102['p_kruskal'] < 0.01 else "*" if cld_tap_102['p_kruskal'] < 0.05 else "ns"
    print(f"  BG11: H = {cld_bg11_102['h_stat']:.3f}, p = {cld_bg11_102['p_kruskal']:.6f} ({sig_bk})")
    print(f"  TAP:  H = {cld_tap_102['h_stat']:.3f}, p = {cld_tap_102['p_kruskal']:.6f} ({sig_tk})")

    print(f"\n  Tukey HSD Compact Letter Display (102 µmol/m²/s):")
    for i in range(5):
        print(f"    {COND_NAMES[i]}: BG11={cld_bg11_102['letters'][i]}  TAP={cld_tap_102['letters'][i]}")
    ...
    COND_LABELS_LATEX = [r'$C_0=1$', r'$C_0=1/2$', r'$C_0=1/4$', r'$C_0=1/8$', r'$C_0=1/16$']
    ...
    for col, (data_bg, data_tap, exp_bg, exp_tap, light_label) in enumerate([
        ...
        ax = fig.add_subplot(gs[0, col]) if ax_row1 is None else fig.add_subplot(gs[0, col], sharey=ax_row1)
        if ax_row1 is None:
            ax_row1 = ax
        for i, (cond, reps) in enumerate(CONDITIONS.items()):
            if all(rep in data_bg.columns for rep in reps):
                ...
                ax.errorbar(data_bg['time_h'], mean_bg, yerr=std_bg,
                            fmt='o', color=COLORS_BG11_FIG[i], markersize=SCATTER_MARKERSIZE,
            ...
            if all(rep in data_tap.columns for rep in reps):
                ...
                ax.errorbar(data_tap['time_h'], mean_tap, yerr=std_tap,
                            fmt='o', color=COLORS_TAP_FIG[i], markersize=SCATTER_MARKERSIZE, markeredgewidth=1.5,
                            capsize=2, capthick=0.8, elinewidth=0.8,
                            label=COND_LABELS_LATEX[i] + ' TAP')
        ax.axvline(x=exp_bg['start'], color='mediumseagreen', linestyle='--', linewidth=2)
        ax.axvline(x=exp_bg['end'], color='mediumseagreen', linestyle='--', linewidth=2,
        ...
        ax.axvline(x=exp_tap['end'], color='royalblue', linestyle='--', linewidth=2,
                   label=f"TAP exponential phase: {exp_tap['start']}-{exp_tap['end']}h")
        ax.set_xlabel('Time (h)')
        if col == 0:
            ax.set_ylabel(r'Biomass (cell mL$^{-1}$)')
        light_title_ab = r'$L_0 = 51\ \mu \mathrm{mol}_{h\nu}\ \mathrm{m}^{-2}\ \mathrm{s}^{-1}$' if col == 0 else r'$L_0 = 102\ \mu \mathrm{mol}_{h\nu}\ \mathrm{m}^{-2}\ \mathrm{s}^{-1}$'
        ...
        for lbl in labels:
            if lbl.startswith('BG-11 exponential'):
                ordered_labels.append(lbl); break
        for lbl in labels:
        ...
        ax.legend(ordered_handles, ordered_labels, loc='upper center',
                  bbox_to_anchor=(0.5, -0.18), ncol=4, frameon=False, fontsize=12)
    ...
    for col, (mu_bg, mu_tap, cld_bg, cld_tap, light_label) in enumerate([
        ...
        ax = fig.add_subplot(gs[1, col]) if ax_row2 is None else fig.add_subplot(gs[1, col], sharey=ax_row2)
        if ax_row2 is None:
            ax_row2 = ax
        ...
        ax.bar(x - width/2, mean_bg, width, yerr=std_bg,
               label='BG-11', color=bg_face, capsize=5,
               edgecolor='mediumseagreen', linewidth=1.5,
               error_kw=dict(ecolor='mediumseagreen', capthick=1.5))
        ax.bar(x + width/2, mean_tap, width, yerr=std_tap,
               label='TAP', color=tap_face, capsize=5,
               edgecolor='teal', linewidth=1.5,
               error_kw=dict(ecolor='teal', capthick=1.5))

        for i in range(5):
            y_b = mean_bg[i] + std_bg[i] + 0.002
            ax.text(x[i] - width/2, y_b, letters_bg[i],
                    ha='center', va='bottom', fontsize=18, fontweight='heavy', color='mediumseagreen')
            y_t = mean_tap[i] + std_tap[i] + 0.002
            ax.text(x[i] + width/2, y_t, letters_tap[i].upper(),
                    ha='center', va='bottom', fontsize=18, fontweight='heavy', color='teal')

        if col == 0:
            ax.set_ylabel(r'$\mu_{max}$ (h$^{-1}$)')
        ax.set_xlabel(r'$C_0$')
        light_title = r'$L_0 = 51\ \mu \mathrm{mol}_{h\nu}\ \mathrm{m}^{-2}\ \mathrm{s}^{-1}$' if col == 0 else r'$L_0 = 102\ \mu \mathrm{mol}_{h\nu}\ \mathrm{m}^{-2}\ \mathrm{s}^{-1}$'
        ...
        ax.text(0.5, -0.25, stats_text, transform=ax.transAxes, fontsize=12,
                ha='center', va='top',
    ...
    for light, mu_bg, mu_tap in [
        ...
        print(f"\nAt {light} µmol/m²/s:")
        print(f"  Mean ratio µmax(TAP/BG11) = {np.mean(mu_ratio):.2f} ± {np.std(mu_ratio, ddof=1):.2f}")
