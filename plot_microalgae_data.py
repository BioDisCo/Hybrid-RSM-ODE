...
def read_Kambe_data(data_folder='data_Kambe'):
    ...
    print(f"✓ Loaded Kambe data: {od_data.shape[0]} conditions, {len(time_data)} time points")
...
def create_combined_figure(time_kambe, od_kambe, all_data_chlamy):
    ...
    for idx_graph in range(4):
        ...
        for C0_idx in range(4):
            ...
            ax.plot(time_kambe[valid], biomass_exp,
                    marker='o', linestyle='none',
                    color=color, markersize=9, alpha=0.6)

        ein_val = ein_kambe[light_idx]
        ax.set_title(
            rf'$L_0$ = {ein_val:.1f} {LIGHT_UNIT}', pad = 18
        )
    ...
    for idx_L0, L0_val in enumerate(L0_unique):
        ...
        for C0_val in C0_values_chlamy:
            ...
            for rep_name, biomass_rep in zip(['A', 'B', 'C'], data['replicates']):
                ...
                ax.plot(t_plot, N_plot,
                        marker=markers_rep[rep_name],
                        linestyle='none',
                        color=color, alpha=0.6, markersize=9)

        ax.set_title(
            rf'$L_0$ = {L0_val:.1f} {LIGHT_UNIT}', pad = 18
        )
        ax.set_xlabel('Time (h)')
        if idx_L0 == 0:
    ...
    for C0 in sorted(colors_C0_chlamy.keys(), reverse=True):
        legend_elements.append(
            Line2D([0], [0], color=colors_C0_chlamy[C0], lw=3, label=rf'$C_0 = {C0:.4g}$')
        )
    ...
    for rep, marker in markers_rep.items():
        legend_elements.append(
            Line2D([0], [0], marker=marker, linestyle='none',
                   color='gray', markersize=10, label=f'Rep {rep}')
        )
    ...
    for C0_idx in range(4):
        ...
        mask = t <= 60
        ax_zoom1.plot(t[mask], N[mask],
                      marker='o', linestyle='none',
                      color=color, markersize=10, alpha=0.7)

    ax_zoom1.set_title(
        rf'Zoom $L_0$ = 386.7 {LIGHT_UNIT}', pad=18
    )
    ...
    for C0_val in C0_values_chlamy:
        ...
        for rep_name, biomass_rep in zip(['A', 'B', 'C'], data['replicates']):
            ...
            mask = t <= 60
            ax_zoom2.plot(t[mask], N[mask],
                          marker=markers_rep[rep_name],
                          linestyle='none',
                          color=color, markersize=10, alpha=0.7)

    ax_zoom2.set_title(
        rf'Zoom $L_0$ = 102 {LIGHT_UNIT}', pad=18
    )
    ...
    if 0.125 in biomass_at_57h_replicates and 1.0 in biomass_at_57h_replicates:
        ...
        if len(N_C0_min_replicates) > 0 and len(N_C0_max_replicates) > 0:
            ...
            percent_increase_zoom2 = ((N_C0_max_mean - N_C0_min_mean) / N_C0_min_mean) * 100
