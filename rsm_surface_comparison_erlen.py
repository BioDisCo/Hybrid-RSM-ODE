...
class RSMModels:
    ...
    def monod_latex(params, param_name):
        return (f"${param_name} = {params[0]:.3e} \\times "
    ...
    def monod_haldane_latex(params, param_name):
        return (f"${param_name} = {params[0]:.2e} \\times "
    ...
    def polynomial_inverse_latex(params, param_name):
        return (f"${param_name} = \\frac{{{params[0]:.2e}}}"
    ...
    def monod_beta0_modulation_initial_params(L0, C0, y):
        return [np.max(y) * 0.8, np.max(y) * 0.4, np.median(C0), np.median(L0), np.median(C0)]
    ...
    def monod_beta0_modulation_latex(params, param_name):
        return (f"${param_name} = ({params[0]:.2e} + \\frac{{{params[1]:.2e} C_0}}{{C_0 + {params[2]:.3f}}}) \\times "
...
def plot_rsm_comparison(df, df_all, response_var, model_name, output_dir):
    ...
    # Fit the RSM surface
    predict_func, coeffs, r2_calib, n_calib, n_params, rss_calib = fit_rsm_surface(df, response_mean, model_name)

    # Compute global R2 on ALL conditions (calibration + extrapolation)
    y_all = df_all[response_mean].values
    y_pred_all = np.array([predict_func(row['L0'], row['C0']) for _, row in df_all.iterrows()])
    r2_global = calculate_r2(y_all, y_pred_all)
    n_global = len(y_all)
    rss_global = np.sum((y_all - y_pred_all) ** 2)

    logger.info(f"  Model {model_name}, {response_var}: R²_calib={r2_calib:.4f}, R²_global={r2_global:.4f}")
    ...
    for L0_val in L0_unique:
        ...
        for _, row in df_L0.iterrows():
            ax1.scatter(row['L0'], row['C0'], row[response_mean],
                       c=[L0_to_color[L0_val]], s=50, linewidth=2, alpha=0.8)
            
            # Vertical error bars
            ax1.plot([row['L0'], row['L0']],
                    [row['C0'], row['C0']],
                    [row[response_mean] - row[response_std], row[response_mean] + row[response_std]],
                    color=L0_to_color[L0_val], linewidth=2, alpha=0.6)
    ...
    logger.info(f"  Model {model_name}, {response_var}: {len(df_calibration)} calibration points, {len(df_extrapolation)} extrapolation points")
    
    # Calibration points (black, circles)
    for idx, row in df_calibration.iterrows():
        ax2.scatter(row['L0'], row['C0'], row[response_mean],
                   color='black', s=50, marker='o', linewidth=1.5, alpha=0.9,
    ...
    for idx, row in df_extrapolation.iterrows():
        ax2.scatter(row['L0'], row['C0'], row[response_mean],
                   color='mediumslateblue', s=100, marker='o', edgecolor='white',
    ...
    ax_text.text(0.5, 0.5, equation_latex,
                ha='center', va='center',
...
def plot_logistic_fits_overview(df_params, output_dir):
    ...
    for _, row in df_params.iterrows():
        ...
        for fit_data in row['replicate_fits']:
            ...
            ax.plot(fit['time_fit'], fit['biomass_fit'], '-',
                   color=colors_rep[rep_name], linewidth=1.5,
                   label=f'{rep_name}: R²={fit["r_squared"]:.2f}')

        # Title with parameters
        title_text = (f"L0={L0:.3f}, C0={C0:.4f}\n"
    ...
    for r in range(nrows):
        ...
        axes[r, 0].set_ylabel(f'L0={L0_values[nrows - 1 - r]:.3f}\nN (cells/mL)', fontsize=9)
    for c in range(ncols):
        # Column label (C0) on top
        axes[0, c].set_title(f'C0={C0_values[c]:.4f}\n' + axes[0, c].get_title(), fontsize=8)
...
def load_and_process_data(files_dict, conv_OD_to_cell, selected_conditions=None):
    ...
    for L0_factor, filepath in files_dict.items():
        ...
        for cond_idx in range(5):
            ...
            for rep_name in ['A', 'B', 'C']:
                ...
                if params and params['r_squared'] > 0.7:
                    ...
                    if params is None or params['r_squared'] <= 0.7:
                        logger.warning(f"  L0={L0_factor}, C0={C0_factor}, Rep {rep_name}: Fitting failed or R² too low")
            ...
            if len(replicate_params['mu_max']) >= 2:
                ...
                logger.info(f"  ✓ L0={L0_factor:.3f}, C0={C0_factor:.4f}: "
                ...
                logger.warning(f"  ✗ L0={L0_factor:.3f}, C0={C0_factor:.4f}: Not enough successful fits ({len(replicate_params['mu_max'])} replicates)")
    ...
    if selected_conditions is not None:
        ...
        df_params = df_params_all[
            df_params_all.apply(lambda row: (row['L0'], row['C0']) in selected_keys, axis=1)
        ].copy()
...
def main():
    ...
    for model_name, model_info in MODEL_LIBRARY.items():
        ...
        r2_mu_calib, r2_mu_global, n_mu_calib, n_mu_global, k_mu, rss_mu_calib, rss_mu_global = \
            plot_rsm_comparison(df_params, df_params_all, 'mu_max', model_name, output_dir)

        # Tester pour N_max
        logger.info("  Fitting N_max...")
        r2_N_calib, r2_N_global, n_N_calib, n_N_global, k_N, rss_N_calib, rss_N_global = \
            plot_rsm_comparison(df_params, df_params_all, 'Nmax', model_name, output_dir)
        ...
        bic_mu = n_mu_calib * np.log(rss_mu_calib / n_mu_calib) + k_mu * np.log(n_mu_calib)
        aicc_mu = aic_mu + 2 * k_mu * (k_mu + 1) / (n_mu_calib - k_mu - 1) if n_mu_calib - k_mu - 1 > 0 else np.inf

        aic_N = n_N_calib * np.log(rss_N_calib / n_N_calib) + 2 * k_N
        bic_N = n_N_calib * np.log(rss_N_calib / n_N_calib) + k_N * np.log(n_N_calib)
        aicc_N = aic_N + 2 * k_N * (k_N + 1) / (n_N_calib - k_N - 1) if n_N_calib - k_N - 1 > 0 else np.inf
    ...
    # Save summary
    df_results.to_csv(output_dir / 'model_comparison_summary.csv', index=False, sep=';', encoding='utf-8-sig')
