...
def load_plate_data(growth_rates_file, steady_states_file):
    ...
    logger.info(f"Loaded {len(df_all)} conditions total "
                f"({len(df_calib)} calibration, {len(df_all) - len(df_calib)} extrapolation)")
    logger.info(f"L0 range: [{df_all['L0'].min()}, {df_all['L0'].max()}]  "
...
def plot_logistic_fits_overview(df_all, output_dir):
    ...
    for row_idx, L0_val in enumerate(reversed(L0_values)):
        ...
        for col_idx, (response, label, unit) in enumerate([
            ...
            for typ in ['calibration', 'extrapolation']:
                ...
                if subset.empty:
                    continue
                ax.scatter(subset['C0'], subset[response],
                           c=colors[typ], marker=markers[typ],
                           s=50, alpha=0.85, label=typ.capitalize())
    ...
    plt.suptitle('Measured growth parameters — plate 96-well\n'
...
def fit_rsm_surface(df, response_mean, model_name):
    ...
    if model_name == 'Monod_Haldane':
        ...
        yaml_path = pathlib.Path("results_plates") / (
            "parameters_growth_rate.yaml" if is_mu_max else "parameters_steady_state.yaml"
        )
        if yaml_path.exists():
            ...
            logger.info(f"  Monod_Haldane: using fitting_plates.py params from {yaml_path.name}")
            ...
            logger.info(f"  Monod_Haldane successful: R² = {r2:.4f} (identical to fitting_plates.py)")
            return predict, coeffs, r2, n_obs, n_params, rss
        else:
            logger.warning(f"  YAML not found at {yaml_path}, falling back to generic p0")
            initial_params = (
...
def plot_rsm_comparison(df, df_all, response_var, model_name, output_dir):
    ...
    predict_func, coeffs, r2_calib, n_calib, n_params, rss_calib = \
        fit_rsm_surface(df, response_mean, model_name)

    y_all       = df_all[response_mean].values
    y_pred_all  = np.array([predict_func(row['L0'], row['C0'])
                             for _, row in df_all.iterrows()])
    r2_global   = calculate_r2(y_all, y_pred_all)
    n_global    = len(y_all)
    rss_global  = np.sum((y_all - y_pred_all) ** 2)

    logger.info(f"  {model_name}, {response_var}: "
    ...
    gs  = fig.add_gridspec(2, 3, height_ratios=[3, 0.4],
    ...
    for L0_val in L0_unique:
        ...
        for _, row in subset.iterrows():
            ax1.scatter(row['L0'], row['C0'], row[response_mean],
                        c=[L0_to_color[L0_val]], s=50, alpha=0.8)
            std_val = row[response_std]
            if not np.isnan(std_val) and std_val > 0:
                ax1.plot([row['L0'], row['L0']],
                         [row['C0'], row['C0']],
                         [row[response_mean] - std_val,
                          row[response_mean] + std_val],
                         color=L0_to_color[L0_val], linewidth=1.5, alpha=0.6)
    ...
    ax2.plot_surface(L0_grid, C0_grid, response_grid,
    ...
    for idx, row in df_calibration.iterrows():
        ...
        std_val = row[response_std]
        if not np.isnan(std_val) and std_val > 0:
            ax2.plot([row['L0'], row['L0']], [row['C0'], row['C0']],
    ...
    if len(df_extrapolation) > 0:
        ax3.scatter(df_extrapolation['L0'], df_extrapolation['C0'],
                    s=100, c='mediumslateblue', marker='o',
...
def main():
    ...
    steady_states_file  = base_dir / "results_plates" / "plates_steady_states_measured.csv"
    ...
    for model_name, model_info in MODEL_LIBRARY.items():
        ...
        aicc_mu = (aic_mu + 2 * k_mu * (k_mu + 1) / (n_mu_calib - k_mu - 1)
                   if n_mu_calib - k_mu - 1 > 0 else np.inf)

        aic_N   = n_N_calib * np.log(rss_N_calib / n_N_calib) + 2 * k_N
        bic_N   = n_N_calib * np.log(rss_N_calib / n_N_calib) + k_N * np.log(n_N_calib)
        aicc_N  = (aic_N + 2 * k_N * (k_N + 1) / (n_N_calib - k_N - 1)
                   if n_N_calib - k_N - 1 > 0 else np.inf)
    ...
    df_results.to_csv(output_dir / 'model_comparison_summary.csv',
