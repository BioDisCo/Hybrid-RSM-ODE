...
def plot_superimposed_surfaces(params):
    ...
    ax1.set_title(
        "Growth Rate $\\mu_{max}(C_0, L_0)$\nPlate: Haldane | Erlen: Double Monod",
        fontsize=12,
        pad=20,
    )
...
def plot_nmax_scaling(params):
    ...
    ax1.plot_surface(
        L0_grid, C0_grid, n_plate, color="darkviolet", alpha=0.6, edgecolor="none"
    )
    ax1.plot_surface(
    ...
    ax3.contour(
        L0_grid, C0_grid, residuals_percent, levels=[0], colors="black", linewidths=2
    )
...
def plot_mumax_scaling(params):
    ...
    ax1.plot_surface(
        L0_grid, C0_grid, mu_plate, color="turquoise", alpha=0.6, edgecolor="none"
    )
    ax1.plot_surface(
...
def plot_individual_figures(params):
    ...
    result_nmax = minimize_scalar(
        objective_nmax, bounds=(0.001, 100.0), method="bounded"
    )
    ...
    mu_plate_scaled_grid = mu_plate_scaled_func(
        C0_grid, L0_factor_grid * s_L_opt, z_opt
    )
    ...
    ax1.set_zlabel(
        "$N_{max}$ (cells/mL)", fontsize=FONT_LABEL_3D, labelpad=30, rotation=90
    )
    ax1.set_title(
        "$N_{max}$ - Original Surfaces\nPlate vs Erlen", fontsize=FONT_TITLE_3D, pad=20
    )
    ...
    ax3.contour(
        L0_grid, C0_grid, residuals_n_percent, levels=[0], colors="black", linewidths=2
    )
    ...
    ax4.set_title(
        "$\\mu_{max}$ - Original Surfaces\nPlate (Haldane) vs Erlen (Monod)",
        fontsize=FONT_TITLE_3D,
        pad=20,
    )
    ...
    ax6.contour(
        L0_grid, C0_grid, residuals_mu_percent, levels=[0], colors="black", linewidths=2
    )
...
def plot_scaled_plate_vs_erlen_data(params):
    ...
    def n_plate_scaled_at(C0_v, L0_v):
        ...
        if has_ki_ss:
            return alpha_opt * haldane_plate(
                C0_v,
                L0_factor_v,
                p_ss["n_max_ref"],
                p_ss["k_c"],
    ...
    r2_mu_calib = 1 - np.sum((mu_obs_calib - mu_pred_calib) ** 2) / np.sum(
        (mu_obs_calib - np.mean(mu_obs_calib)) ** 2
    )

    # mu_max R² global (all points)
    mu_obs_all = df["mu_max_mean"].values
    mu_pred_all = mu_plate_scaled_at(df["C0"].values, df["L0"].values)
    r2_mu_global = 1 - np.sum((mu_obs_all - mu_pred_all) ** 2) / np.sum(
        (mu_obs_all - np.mean(mu_obs_all)) ** 2
    )
    ...
    r2_n_calib = 1 - np.sum((n_obs_calib - n_pred_calib) ** 2) / np.sum(
        (n_obs_calib - np.mean(n_obs_calib)) ** 2
    )

    # N_max R² global (all points)
    n_obs_all = df["Nmax_mean"].values
    n_pred_all = n_plate_scaled_at(df["C0"].values, df["L0"].values)
    r2_n_global = 1 - np.sum((n_obs_all - n_pred_all) ** 2) / np.sum(
        (n_obs_all - np.mean(n_obs_all)) ** 2
    )

    logger.info(
        f"  mu_max: R² calib = {r2_mu_calib:.4f}, R² global = {r2_mu_global:.4f}"
    )
    ...
    ax_mu.set_zlabel(
        r"$\mu_{max}$ (h$^{-1}$)", fontsize=FONT_LABEL_3D, labelpad=30, rotation=90
    )
    ax_mu.set_title(
        f"$\\mu_{{max}}$ - Scaled Plate vs Erlen Data\n"
    ...
    z_mumax_min = min(
        mu_plate_scaled_grid.min(), df["mu_max_mean"].min() - df["mu_max_std"].max()
    )
    z_mumax_max = max(
        mu_plate_scaled_grid.max(), df["mu_max_mean"].max() + df["mu_max_std"].max()
    )
    ...
    ax_n.set_title(
        f"$N_{{max}}$ - Scaled Plate vs Erlen Data\n"
...
def plot_plate_erlen_points_comparison(params):
    ...
    # =========================================================================
    # Erlen data
    df_erlen = pd.read_csv(
        ERLEN_DIR / "growth_parameters_all_25_conditions.csv", sep=";"
    )
    ...
    logger.info(
        f"Loaded {len(df_plate_mu)} plate mu_max points, {len(df_plate_n)} plate N_max points"
    )
    ...
    logger.info(
        f"  Scaling parameters: alpha={alpha_opt:.4f}, s_L={s_L_opt:.4f}, z={z_opt:.4f}"
    )
    ...
    # Round for matching
    erlen_keys = {
        (round(r["C0"], 4), round(r["L0"], 1)): r for _, r in df_erlen.iterrows()
    }
            ...
            comparison_rows.append(
                {
                    ...
                    "mu_max_error_rel_pct": 100
                    * abs(row_erlen["mu_max_mean"] - row_plate_mu["mu_max_transformed"])
                    / row_erlen["mu_max_mean"],
                    "N_max_erlen": row_erlen["Nmax_mean"],
                    "N_max_plate_transformed": row_plate_n["N_max_transformed"],
                    "N_max_error_rel_pct": 100
                    * abs(row_erlen["Nmax_mean"] - row_plate_n["N_max_transformed"])
                    / row_erlen["Nmax_mean"],
                    "type": row_erlen["type"],
                }
            )
    ...
    r2_mu_calib = compute_r2(
        df_calib["mu_max_erlen"].values, df_calib["mu_max_plate_transformed"].values
    )
    r2_mu_extrap = compute_r2(
        df_extrap["mu_max_erlen"].values, df_extrap["mu_max_plate_transformed"].values
    )
    r2_mu_global = compute_r2(
        df_comp["mu_max_erlen"].values, df_comp["mu_max_plate_transformed"].values
    )

    r2_n_calib = compute_r2(
        df_calib["N_max_erlen"].values, df_calib["N_max_plate_transformed"].values
    )
    r2_n_extrap = compute_r2(
        df_extrap["N_max_erlen"].values, df_extrap["N_max_plate_transformed"].values
    )
    r2_n_global = compute_r2(
        df_comp["N_max_erlen"].values, df_comp["N_max_plate_transformed"].values
    )

    logger.info(
        f"  mu_max R²: calib={r2_mu_calib:.4f}, extrap={r2_mu_extrap:.4f}, global={r2_mu_global:.4f}"
    )
    logger.info(
        f"  N_max  R²: calib={r2_n_calib:.4f}, extrap={r2_n_extrap:.4f}, global={r2_n_global:.4f}"
    )
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "type",
                "mu_max_erlen",
                "mu_max_plate_transformed",
                "mu_max_error_rel_pct",
                "N_max_erlen",
                "N_max_plate_transformed",
                "N_max_error_rel_pct",
            ]
        )
        for _, row in df_comp.iterrows():
            writer.writerow(
                [
                    ...
                    f"{row['L0']:.1f}",
                    row["type"],
                    f"{row['mu_max_erlen']:.6e}",
                    f"{row['mu_max_plate_transformed']:.6e}",
                    f"{row['mu_max_error_rel_pct']:.2f}",
                    f"{row['N_max_erlen']:.6e}",
                    f"{row['N_max_plate_transformed']:.6e}",
        ...
        writer.writerow([])
        writer.writerow(
            ["# Global R2 summary", "", "", "mu_max_R2", "", "", "N_max_R2", "", ""]
        )
        writer.writerow(
            [
                "calibration",
    ...
    ax_mu.scatter(
        df_p_mu_calib["L0_actual"].values,
        df_p_mu_calib["C0"].values,
    ...
    ax_mu.scatter(
        df_p_mu_extrap["L0_actual"].values,
        df_p_mu_extrap["C0"].values,
    ...
    ax_mu.scatter(
        df_e_calib["L0"].values,
        df_e_calib["C0"].values,
    ...
    ax_mu.scatter(
        df_e_extrap["L0"].values,
        df_e_extrap["C0"].values,
    ...
    ax_mu.set_title(
        f"$\\mu_{{max}}$ - Plate Transformed vs Erlen\n"
    ...
    all_mu = np.concatenate(
        [df_plate_mu["mu_max_transformed"].values, df_erlen["mu_max_mean"].values]
    )
    ...
    # Plate transformed: calibration (blue circles) and extrapolation (blue triangles)
    ax_n.scatter(
        df_p_n_calib["L0_actual"].values,
        df_p_n_calib["C0"].values,
    ...
    ax_n.scatter(
        df_p_n_extrap["L0_actual"].values,
        df_p_n_extrap["C0"].values,
    ...
    ax_n.set_title(
        f"$N_{{max}}$ - Plate Transformed vs Erlen\n"
    ...
    all_n = np.concatenate(
        [df_plate_n["N_max_transformed"].values, df_erlen["Nmax_mean"].values]
    )
...
def main():
    ...
    print(
        f"  mu_max: mu_max_ref={params['plate_gr']['mu_max_ref']:.4e}, "
        f"k_c={params['plate_gr']['k_c']:.4f}, k_l={params['plate_gr']['k_l']:.4f}",
        end="",
    )
    if "k_i" in params["plate_gr"]:
    ...
    print(
        f"  N_max:  n_max_ref={params['plate_ss']['n_max_ref']:.4e}, "
        f"k_c={params['plate_ss']['k_c']:.4f}, k_l={params['plate_ss']['k_l']:.4f}",
        end="",
    )
    if "k_i" in params["plate_ss"]:
    ...
    print(
        f"  mu_max: mu_max_ref={params['erlen_gr']['mu_max_ref']:.4e}, "
        f"k_c={params['erlen_gr']['k_c']:.4f}, k_l={params['erlen_gr']['k_l']:.2f}"
    )
    print(
        f"  N_max:  n_max_ref={params['erlen_ss']['n_max_ref']:.4e}, "
    ...
    print(
        f"  mu_max_erlen ≈ {mu_scaling_results['z']:.4f} × mu_max_plate(C0, {mu_scaling_results['s_L']:.4f} × L0)"
    )
